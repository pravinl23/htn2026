// Ghost host: the ONLY code inside Ghost.app, and it never changes. macOS ties the Accessibility grant of an
// ad-hoc signed app to its code hash, so everything that does change lives in libghost.dylib OUTSIDE the
// bundle (docs/desktop-realworld.md section 1). Do not add anything here: a rebuilt host loses the grant.
//
// Library: $GHOST_LIB (when set it is the only candidate), else ~/Library/Application Support/Ghost/libghost.dylib,
// else <bundle>/../libghost.dylib, else the path written in ~/Library/Application Support/Ghost/lib-path.txt.
...
int main(int argc, const char **argv) {
    @autoreleasepool {
        NSString *support = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Application Support/Ghost"];
        NSMutableArray<NSString *> *candidates = [NSMutableArray array];
        const char *env = getenv("GHOST_LIB");
        if (env && *env) {
            [candidates addObject:@(env)];
        } else {
            [candidates addObject:[support stringByAppendingPathComponent:@"libghost.dylib"]];
            [candidates addObject:[NSBundle.mainBundle.bundlePath.stringByDeletingLastPathComponent stringByAppendingPathComponent:@"libghost.dylib"]];
        ...
        if (!path) {
            return Fail(argc, argv, [NSString stringWithFormat:@"libghost.dylib not found. Looked at:\n%@\nBuild it with `make -C desktop lib` (and `make -C desktop install-lib`).",
