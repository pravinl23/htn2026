...
//
//   fixture -> SBCapture -> shabang-core.js -> SBController -> SBWriter -> SBOpenPanelDriver / SBComboBoxDriver
//
// A fake "Safari" reacts to everything Shabang does: AXScrollToVisible scrolls the page, pressing Attach hangs the
...
// Keys only ever reach SBFakeKeyPoster (the production guard logic over a recording sink); a hand-driven clock runs
...
@interface GWActuator : SBFakeAXActuator
@property (nonatomic, weak) GWWorld *world;
@end

@interface GWWorld : NSObject
@property (nonatomic) pid_t pid;
@property (nonatomic, strong) SBFakeDesktopState *state;
...
@property (nonatomic, strong) SBFakeAXNode *window, *scrollArea, *web;
@property (nonatomic, strong) SBFakeAXNode *panel, *goToSheet, *fileList, *openButton, *goToField;
@property (nonatomic, weak) SBFakeAXNode *uploadWidget;       // the widget whose Attach opened the panel
@property (nonatomic, copy) NSString *expectedPath;
@property (nonatomic, weak) SBFakeAXNode *selectedAll;
@property (nonatomic, strong, nullable) SBFakeAXNode *menu;
@property (nonatomic, weak, nullable) SBFakeAXNode *menuCombo;
...
/// Greenhouse's react-select does for the EEO selects (SBComboBoxDriver step 3a). The phone-prefix Country
...
@property (nonatomic, weak, nullable) SBFakeAXNode *replacedOnWrite;
@property (nonatomic) BOOL replacementKeepsValue;
@property (nonatomic, copy, nullable) void (^afterPost)(SBKeyStroke *stroke);
- (void)focus:(nullable SBFakeAXNode *)node;
- (void)replaceFieldNode:(SBFakeAXNode *)node withValue:(NSString *)value;
- (BOOL)panelOpen;
- (void)openPanelFrom:(SBFakeAXNode *)button;
- (void)openMenuFor:(SBFakeAXNode *)combo;
- (void)choose:(NSString *)option;
@end

static BOOL GWInside(id<SBAXNode> node, id<SBAXNode> ancestor) {
    for (id<SBAXNode> up = node; up; up = up.parent) if (up == ancestor) return YES;
    return NO;
}

static SBFakeAXNode *GWFind(id<SBAXNode> root, BOOL (^match)(SBFakeAXNode *node)) {
    if (match((SBFakeAXNode *)root)) return (SBFakeAXNode *)root;
    for (id<SBAXNode> child in root.children) {
        SBFakeAXNode *hit = GWFind(child, match);
        if (hit) return hit;
    }
    return nil;
}

static void GWCollect(id<SBAXNode> root, NSMutableArray<SBFakeAXNode *> *out) {
    [out addObject:(SBFakeAXNode *)root];
    for (id<SBAXNode> child in root.children) GWCollect(child, out);
}

static SBFakeAXNode *GWNode(NSString *role, NSString *title, CGRect frame) {
...
- (BOOL)pressNode:(id<SBAXNode>)node {
    ...
    GWWorld *world = self.world;
    SBFakeAXNode *fake = (SBFakeAXNode *)node;
    if (world.menu && GWInside(node, world.menu)) {
        [world choose:[SBComboBoxDriver textOfOption:node]];
        return YES;
    }
    if ([SBOpenPanelDriver isUploadButton:node]) [world openPanelFrom:fake];
...
- (instancetype)init {
    ...
    _window = [SBFakeAXNode nodeWithDumpTreeFile:GWFixturePath()];
    if (!_window) return nil;
    _web = GWFind(_window, ^BOOL(SBFakeAXNode *node) { return [node.role isEqualToString:@"AXWebArea"]; });
    _scrollArea = (SBFakeAXNode *)_web.parent;
    _pid = 4711;
    _state = [[SBFakeDesktopState alloc] init];
    ...
    SBFakeAXNode *search = [_panel addChild:GWNode(@"AXTextField", nil, CGRectMake(900, 160, 180, 24))];
    ...
    _actuator.onScroll = ^(SBFakeAXNode *node) { [weakSelf scrollToShow:node]; };
    return self;
}
...
- (void)scrollToShow:(SBFakeAXNode *)node {
    ...
    self.scrolls++;
    NSMutableArray<SBFakeAXNode *> *all = [NSMutableArray array];
    GWCollect(self.web, all);
    for (SBFakeAXNode *each in all) each.frame = CGRectOffset(each.frame, 0, delta);
}
...
- (void)openPanelFrom:(SBFakeAXNode *)button {
    ...
    SBFakeAXNode *widget = (SBFakeAXNode *)button.parent;
    while (widget && widget.title.length == 0) widget = (SBFakeAXNode *)widget.parent;
...
/// React replaces an input while Shabang writes into it: the old element dies, an identical one (same role, label and
/// DOM identifier, so the same signature) takes its place, with or without the value that was just written.
- (void)replaceFieldNode:(SBFakeAXNode *)node withValue:(NSString *)value {
    SBFakeAXNode *parent = (SBFakeAXNode *)node.parent;
    NSUInteger index = [parent indexOfChild:node];
    if (index == NSNotFound) return;
    SBFakeAXNode *fresh = [SBFakeAXNode nodeWithRole:node.role title:node.title frame:node.frame];
...
- (void)closePanelWithFile:(BOOL)chosen {
    ...
    SBFakeAXNode *widget = self.uploadWidget;
    SBFakeAXNode *attach = GWFind(widget, ^BOOL(SBFakeAXNode *node) { return [node.title isEqualToString:@"Attach"]; });
    ...
    if (chosen && self.fileShownOutsideWidget) {
        ...
        [self.web addChild:[SBFakeAXNode staticText:[self.expectedPath.lastPathComponent stringByAppendingString:@" uploaded"]
...
- (void)showFile:(NSString *)name inWidget:(SBFakeAXNode *)widget {
    if (!widget || name.length == 0) return;
    if (self.pageDropsUploadControls) {
        NSMutableArray<SBFakeAXNode *> *inside = [NSMutableArray array];
        GWCollect(widget, inside);
        for (SBFakeAXNode *node in inside) {
            if (![node.title isEqualToString:@"Attach"] && ![SBOpenPanelDriver isUploadButton:node]) continue;
            [(SBFakeAXNode *)node.parent removeChild:node];
            [self.actuator.goneNodes addObject:node];
        }
    }
    CGRect box = widget.frame;
    SBFakeAXNode *row = [SBFakeAXNode nodeWithRole:@"AXGroup" title:nil frame:CGRectMake(box.origin.x, box.origin.y + 30, 400, 24)];
    [row addChild:[SBFakeAXNode staticText:name frame:CGRectMake(box.origin.x, box.origin.y + 32, 200, 20)]];
...
- (void)openMenuFor:(SBFakeAXNode *)combo {
    ...
    SBFakeAXNode *menu = [SBFakeAXNode nodeWithRole:@"AXList" title:nil frame:CGRectMake(combo.frame.origin.x, CGRectGetMaxY(combo.frame) + 6, 537, 160)];
    ...
    for (NSString *option in self.optionsByCombo[combo.title] ?: @[]) {
        ...
        [menu addChild:[SBFakeAXNode staticText:option frame:CGRectMake(combo.frame.origin.x, CGRectGetMaxY(combo.frame) + 8 + 24 * shown, 520, 22)]];
        shown++;
    }
    if (shown == 0) [menu addChild:[SBFakeAXNode staticText:@"No options" frame:CGRectZero]];
    else ((SBFakeAXNode *)menu.children.firstObject).isFocused = YES;   // react-select highlights the first option
    SBFakeAXNode *parent = (SBFakeAXNode *)combo.parent;
...
- (void)closeMenu {
    if (self.menu) [(SBFakeAXNode *)self.menu.parent removeChild:self.menu];
    self.menu = nil;
    self.menuCombo = nil;
}

- (void)choose:(NSString *)option {
    SBFakeAXNode *combo = self.menuCombo;
    [self closeMenu];
    if (!combo) return;
    ...
    SBFakeAXNode *parent = (SBFakeAXNode *)combo.parent;
    NSUInteger index = [parent indexOfChild:combo];
    SBFakeAXNode *before = index > 0 ? (SBFakeAXNode *)parent.children[index - 1] : nil;
    BOOL placeholder = NO;
    ...
    if (placeholder) {
        ...
        ((SBFakeAXNode *)before.children.firstObject).value = option;
    } else {
        SBFakeAXNode *value = [SBFakeAXNode nodeWithRole:@"AXGroup" title:nil frame:CGRectMake(combo.frame.origin.x, combo.frame.origin.y, 120, combo.frame.size.height)];
        value.domClassList = @[ @"select__single-value" ];
        [value addChild:[SBFakeAXNode staticText:option frame:value.frame]];
...
- (NSString *)nameOf:(SBFakeAXNode *)node {
    ...
    switch (stroke.kind) {
        ...
        case SBKeyStrokeKindReturn:
            if (focused == self.goToField && [self goToOpen]) {
                ...
                SBFakeAXNode *highlighted = nil;
                for (id<SBAXNode> option in self.menu.children) if (option.isFocused) highlighted = (SBFakeAXNode *)option;
...
+ (instancetype)rig {
    ...
    SBWriter *writer = [[SBWriter alloc] initWithActuator:world.actuator];
    writer.after = world.clock.after;
    SBCapture *capture = rig.capture;
    writer.isNodeSensitive = ^BOOL(id<SBAXNode> node) { return [capture isNodeSensitive:node]; };
    SBOpenPanelDriver *panel = [[SBOpenPanelDriver alloc] initWithActuator:world.actuator poster:world.poster state:world.state];
    panel.after = world.clock.after;
    panel.clock = world.clock.clock;
    SBComboBoxDriver *combo = [[SBComboBoxDriver alloc] initWithActuator:world.actuator poster:world.poster state:world.state];
    ...
    SBController *controller = [[SBController alloc] initWithCore:rig.core store:rig.store client:nil];
    ...
    controller.overlay = [[SBOverlayWindow alloc] initWithLayout:[SBScreenLayout layoutWithFrames:@[ [NSValue valueWithRect:NSMakeRect(0, 0, 1470, 956)] ] scales:@[ @2 ]]];
    ...
    SBFakeAXNode *window = world.window;
    controller.captureProvider = ^SBCaptureResult *{ return [capture captureWindow:window]; };
    ...
    controller.focusedNodeProvider = ^id<SBAXNode> { return weakWorld.state.focusedNode; };
...
- (BOOL)tab {
    ...
    BOOL consumed = [self.controller.eventTap handleKeyDown:SBKeyCodeTab flags:0 isRepeat:NO userData:0 printable:NO];
    SBController *controller = self.controller;
...
/// A repeat of a held Tab (the tap already decided it belongs to Shabang), then every timer runs out.
- (void)hold {
    [self.controller eventTap:self.controller.eventTap didConsumeTab:SBKeyDecisionAccept isRepeat:YES];
    SBController *controller = self.controller;
...
- (SBFakeAXNode *)comboTitled:(NSString *)title {
    return GWFind(self.world.window, ^BOOL(SBFakeAXNode *node) { return [node.role isEqualToString:@"AXComboBox"] && [node.title isEqualToString:title]; });
}

- (SBFakeAXNode *)buttonTitled:(NSString *)title {
    return GWFind(self.world.window, ^BOOL(SBFakeAXNode *node) { return [node.role isEqualToString:@"AXButton"] && [node.title isEqualToString:title]; });
}

- (SBFakeAXNode *)textFieldTitled:(NSString *)title {
    return GWFind(self.world.window, ^BOOL(SBFakeAXNode *node) { return [node.role isEqualToString:@"AXTextField"] && [node.title isEqualToString:title]; });
}
...
GH_TEST(integration_greenhouse_tab_walk_fills_uploads_chooses_and_parks_on_submit) {
    ...
    [rig rescan];
    SBWalkState *walk = controller.walk;
    NSMutableDictionary<NSString *, NSString *> *labels = [NSMutableDictionary dictionary];
    for (SBField *field in [rig.capture captureWindow:world.window].fields) labels[field.signature] = field.label;
    NSMutableArray<NSString *> *ghosted = [NSMutableArray array];
    for (SBGhost *ghost in walk.ghosts) [ghosted addObject:[NSString stringWithFormat:@"%@ %@", ghost.action, labels[ghost.signature]]];
    ...
    // Every proposal Shabang is not certain of wears the badge and stops a held accept key; what it knows
    ...
    for (SBGhost *ghost in walk.ghosts) {
    ...
    for (SBGhost *ghost in walk.ghosts) {
        if ([ghost.action isEqualToString:SBGhostActionSelect]) GH_ASSERT(ghost.lazy);
        if ([ghost.action isEqualToString:SBGhostActionUpload]) GH_ASSERT_EQUAL_OBJECTS(ghost.displayText, @"resume-alex-chen.pdf");
    }
    ...
    GH_ASSERT_EQUAL_INT([world.poster countOfKind:SBKeyStrokeKindGoToFolder], 1);
    ...
    // Dropbox...).
    SBFakeAXNode *submit = [rig buttonTitled:@"Submit application"];
    NSMutableArray<NSString *> *pressed = [NSMutableArray array];
    for (id<SBAXNode> node in world.actuator.pressedNodes) [pressed addObject:node.title ?: node.value ?: node.role];
    ...
    // itself offers, so no wording of Shabang's ever lands in a demographic field (docs/answers.md section 7).
    for (NSString *title in @[ @"Gender", @"Are you Hispanic/Latino?", @"Veteran Status", @"Disability Status" ]) {
        SBFakeAXNode *combo = [rig comboTitled:title];
    ...
    for (int i = 0; i < 3; i++) [controller eventTap:controller.eventTap didConsumeTab:SBKeyDecisionAccept isRepeat:YES];
    ...
    SBCaptureResult *after = [rig.capture captureWindow:world.window];
    for (SBField *field in after.fields) {
...
GH_TEST(integration_hold_tab_stops_at_a_combobox_or_upload_and_a_fresh_press_starts_it) {
    ...
    GH_ASSERT_EQUAL_INT([world.poster countOfKind:SBKeyStrokeKindGoToFolder], 0);
    GH_ASSERT([rig tab]);
    GH_ASSERT_EQUAL_OBJECTS(world.panelsOpenedBy, (@[ @"Attach" ]));
...
GH_TEST(integration_a_user_key_during_the_upload_aborts_it_and_queued_tabs_are_dropped) {
    ...
    world.afterPost = ^(SBKeyStroke *stroke) {
        if (stroke.kind != SBKeyStrokeKindGoToFolder || pressed) return;
        pressed = YES;
        ...
        GH_ASSERT([tap handleKeyDown:SBKeyCodeTab flags:0 isRepeat:NO userData:0 printable:NO]);   // queued, not native
        ...
        panelTabConsumed = [tap handleKeyDown:SBKeyCodeTab flags:0 isRepeat:NO userData:0 printable:NO];
    };
    [rig tab];
    GH_ASSERT(pressed);
...
GH_TEST(integration_losing_the_permission_mid_upload_cancels_it_before_another_key) {
    ...
    SBController *controller = rig.controller;
    world.afterPost = ^(SBKeyStroke *stroke) {
        if (stroke.kind != SBKeyStrokeKindGoToFolder) return;
        // Accessibility is revoked (or Shabang is switched off) right after Command+Shift+G went out.
        [controller accessibility:controller.accessibility trustDidChange:SBTrustStateUntrusted];
    };
    NSUInteger before = world.poster.posted.count;                // the Country combobox typed earlier in the walk
    NSUInteger returns = [world.poster countOfKind:SBKeyStrokeKindReturn];
    [rig tab];
    ...
    GH_ASSERT_EQUAL_INT([world.poster countOfKind:SBKeyStrokeKindReturn], returns);
...
GH_TEST(integration_an_upload_the_page_shows_late_is_still_accepted) {
    ...
    GH_ASSERT(GWFind(dropped.world.web, ^BOOL(SBFakeAXNode *node) { return [node.title isEqualToString:@"Remove file"]; }) != nil);
...
/// Live, Safari: Greenhouse replaced the First Name input while Shabang wrote into it, and the write was reported as
...
GH_TEST(integration_a_field_the_page_replaces_mid_write_is_not_lost) {
    for (NSUInteger keepsValue = 0; keepsValue < 2; keepsValue++) {
        ...
        SBFakeAXNode *input = GWFind(world.web, ^BOOL(SBFakeAXNode *node) { return [node.title isEqualToString:@"First Name"]; });
        GH_ASSERT(input != nil);
        ...
        SBFakeAXNode *now = GWFind(world.web, ^BOOL(SBFakeAXNode *node) { return [node.title isEqualToString:@"First Name"]; });
...
GH_TEST(integration_a_combobox_without_the_answer_is_skipped_and_the_walk_goes_on) {
    ...
    for (SBGhost *ghost in rig.controller.walk.ghosts) GH_ASSERT_FALSE(ghost.lazy && [ghost.displayText isEqualToString:@"Canada"]);
}
