// SBTest: a tiny test harness (no XCTest on this machine).
//
//   #import "SBTest.h"
...
typedef void (*SBTestFunction)(void);
...
#define GH_TEST(name) \
    ...
    __attribute__((constructor)) static void ghtest_register_##name(void) { SBTestRegister(#name, __FILE__, ghtest_##name); } \
    static void ghtest_##name(void)

#define GH_FAIL(...) \
    do { SBTestFail(__FILE__, __LINE__, [NSString stringWithFormat:__VA_ARGS__]); return; } while (0)

// Variadic so that Objective-C literals with commas (@{ a: b, c: d }, @[ x, y ]) need no extra parentheses.
#define GH_ASSERT(...) \
    do { if (!(__VA_ARGS__)) { SBTestFail(__FILE__, __LINE__, @"expected true: " #__VA_ARGS__); return; } } while (0)

#define GH_ASSERT_FALSE(...) \
    do { if ((__VA_ARGS__)) { SBTestFail(__FILE__, __LINE__, @"expected false: " #__VA_ARGS__); return; } } while (0)

#define GH_ASSERT_MSG(condition, ...) \
    do { if (!(condition)) { SBTestFail(__FILE__, __LINE__, [NSString stringWithFormat:__VA_ARGS__]); return; } } while (0)

#define GH_ASSERT_EQUAL_OBJECTS(actual, expected) \
    do { \
        id gh_a = (actual); id gh_e = (expected); \
        if (!(gh_a == gh_e || [gh_a isEqual:gh_e])) { \
            SBTestFail(__FILE__, __LINE__, [NSString stringWithFormat:@"%s: got %@, expected %@", #actual, gh_a, gh_e]); return; \
        } \
    } while (0)
...
#define GH_ASSERT_EQUAL_INT(actual, expected) \
    do { \
        ...
        if (gh_a != gh_e) { SBTestFail(__FILE__, __LINE__, [NSString stringWithFormat:@"%s: got %lld, expected %lld", #actual, gh_a, gh_e]); return; } \
    } while (0)

#define GH_ASSERT_NEAR(actual, expected, tolerance) \
    do { \
        double gh_a = (double)(actual); double gh_e = (double)(expected); \
        if (fabs(gh_a - gh_e) > (tolerance)) { SBTestFail(__FILE__, __LINE__, [NSString stringWithFormat:@"%s: got %g, expected %g", #actual, gh_a, gh_e]); return; } \
    } while (0)
