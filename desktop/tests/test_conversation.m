// SBConversation: reading a thread off the screen, against a tree shaped like a real Messages window.
//
...
static SBFakeAXNode *CVNode(NSString *role, CGRect frame) {
...
static void CVAddMessage(SBFakeAXNode *thread, NSString *described, CGFloat x, CGFloat y, CGFloat width) {
    SBFakeAXNode *row = [thread addChild:CVNode(@"AXGroup", CGRectMake(x, y, width, 33))];
    row.axDescription = described;
    SBFakeAXNode *inner = [row addChild:CVNode(@"AXGroup", CGRectMake(x, y, width, 33))];
...
static SBFakeAXNode *CVMessagesWindowWithSidebar(void) {
    SBFakeAXNode *window = CVNode(@"AXWindow", CGRectMake(0, 0, 1470, 806));
...
GH_TEST(conversation_is_empty_where_there_is_no_thread) {
    ...
    SBFakeAXNode *window = CVNode(@"AXWindow", CGRectMake(0, 0, 900, 600));
    SBFakeAXNode *group = [window addChild:CVNode(@"AXGroup", CGRectMake(0, 0, 900, 600))];
    group.axDescription = @"First name, last name, and email";
    [group addChild:CVNode(@"AXTextField", CGRectMake(10, 10, 300, 30))];
    SBConversation *conversation = [SBConversation conversationFromNode:window];
...
GH_TEST(conversation_description_parsing_needs_a_name_words_and_a_time) {
    GH_ASSERT([SBConversation messageFromDescription:@"Sam, on my way, 9:41 AM"] != nil);
    GH_ASSERT([SBConversation messageFromDescription:@"Sam, on my way, 21:41"] != nil);
    GH_ASSERT([SBConversation messageFromDescription:@"Sam, on my way, 9:41 a.m."] != nil);
    // No time on the end: ordinary prose, a label, a summary. Never a message.
    GH_ASSERT([SBConversation messageFromDescription:@"Sam, on my way, tomorrow"] == nil);
    GH_ASSERT([SBConversation messageFromDescription:@"Search, Messages"] == nil);
    GH_ASSERT([SBConversation messageFromDescription:@"9:41 AM"] == nil);
    GH_ASSERT([SBConversation messageFromDescription:nil] == nil);
    // A name and a time with nothing between them is not a message either.
    GH_ASSERT([SBConversation messageFromDescription:@"Sam, , 9:41 AM"] == nil);
}
...
GH_TEST(conversation_keeps_only_the_last_messages) {
    ...
    SBConversation *conversation = [SBConversation conversationFromNode:window];
    GH_ASSERT(conversation.messages.count <= SBConversationMaxMessages);
...
GH_TEST(conversation_outgoing_test_needs_both_edges) {
    ...
    GH_ASSERT([SBConversation frameLooksOutgoing:CGRectMake(1300, 10, 144, 33) inThread:thread]);
    GH_ASSERT_FALSE([SBConversation frameLooksOutgoing:CGRectMake(356, 10, 300, 33) inThread:thread]);
    // A message that spans the whole thread belongs to nobody in particular.
    GH_ASSERT_FALSE([SBConversation frameLooksOutgoing:thread inThread:thread]);
    GH_ASSERT_FALSE([SBConversation frameLooksOutgoing:CGRectZero inThread:thread]);
    GH_ASSERT_FALSE([SBConversation frameLooksOutgoing:CGRectMake(1300, 10, 144, 33) inThread:CGRectZero]);
}
