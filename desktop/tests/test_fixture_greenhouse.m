...
// SBFakeAXNode -> SBCapture -> the real shabang-core.js, so the real page is a regression test.
...
static SBCore *FixtureCore(void) {
    static SBCore *core;
...
static SBCaptureResult *CaptureGreenhouse(void) {
    SBFakeAXNode *window = [SBFakeAXNode nodeWithDumpTreeFile:FixturePath(@"greenhouse-safari-viam.json")];
    if (!window || !FixtureCore()) return nil;
    SBCapture *capture = [[SBCapture alloc] initWithSafety:FixtureCore()];
...
static NSArray<NSDictionary *> *OfflineGhosts(NSArray<SBField *> *fields, NSDictionary *profile) {
    SBCore *core = FixtureCore();
...
static NSDictionary<NSString *, NSString *> *LabelsBySignature(NSArray<SBField *> *fields) {
    NSMutableDictionary *labels = [NSMutableDictionary dictionary];
    for (SBField *field in fields) labels[field.signature] = field.label;
    return labels;
}

static SBField *Labelled(NSArray<SBField *> *fields, NSString *label) {
    for (SBField *field in fields) if ([field.label isEqualToString:label]) return field;
    return nil;
}

static SBField *CoreField(NSString *signature, NSString *label, NSString *kind) {
...
static SBField *LazySelect(NSString *signature, NSString *label) {
    SBField *field = CoreField(signature, label, SBKindSelect);
    field.lazyOptions = YES;
    return field;
}

static SBField *Upload(NSString *signature, NSString *label, NSString *_Nullable uploadKind) {
    SBField *field = CoreField(signature, label, SBKindFile);
...
GH_TEST(fixture_greenhouse_capture_yields_the_form_in_reading_order) {
    SBCaptureResult *result = CaptureGreenhouse();
    ...
    for (SBField *field in result.fields) {
        ...
        if ([field.kind isEqualToString:SBKindButton] && [field.label isEqualToString:@"Apply"]) continue; // top of the page
    ...
    NSArray *expectedKinds = @[
        SBKindText, SBKindText, SBKindEmail, SBKindSelect, SBKindTel, SBKindFile, SBKindFile,
        SBKindURL, SBKindURL, SBKindURL, SBKindSelect, SBKindSelect,
        SBKindSelect, SBKindSelect, SBKindSelect, SBKindSelect, SBKindButton,
    ];
    GH_ASSERT_EQUAL_OBJECTS(kinds, expectedKinds);

    NSArray<SBField *> *fields = result.fields;
    ...
    for (NSString *label in @[ @"Country", @"How did you hear about this opportunity at Viam?", @"Gender", @"Disability Status" ]) {
        SBField *select = Labelled(fields, label);
    ...
    SBField *resume = Labelled(fields, @"Resume/CV");
    GH_ASSERT_EQUAL_OBJECTS(resume.uploadKind, SBUploadKindResume);
    GH_ASSERT_EQUAL_OBJECTS(resume.identifier, @"resume");
    GH_ASSERT_EQUAL_OBJECTS([(SBFakeAXNode *)[result nodeForSignature:resume.signature] title], @"Attach");
    GH_ASSERT_EQUAL_OBJECTS([result uploadNodeForSignature:resume.signature].identifier, @"resume");
    GH_ASSERT_EQUAL_OBJECTS(Labelled(fields, @"Cover Letter").uploadKind, SBUploadKindCoverLetter);
    ...
    NSData *wire = [NSJSONSerialization dataWithJSONObject:[SBField wireJSONObjectsForFields:fields] options:0 error:NULL];
...
GH_TEST(fixture_greenhouse_capture_is_stable_across_replays) {
    SBCaptureResult *first = CaptureGreenhouse(), *second = CaptureGreenhouse();
    ...
    for (SBField *field in first.fields) [a addObject:field.signature];
    for (SBField *field in second.fields) [b addObject:field.signature];
...
GH_TEST(fixture_greenhouse_ghosts_for_alex_chen_with_a_resume) {
    ...
    // never a characteristic Shabang invented. The option list does not exist yet, so the ghost says "match
...
GH_TEST(fixture_greenhouse_submit_is_gated_until_every_required_field_is_answered) {
    ...
    for (SBField *field in result.fields) if (field.required) [required addObject:field.label];
    ...
    for (SBField *field in result.fields) if (field.required) [accepted addObject:field.signature];
...
GH_TEST(fixture_greenhouse_a_correction_changes_the_next_proposal) {
    ...
    SBCore *core = FixtureCore();
    NSDictionary *profile = ProfileWith(@{ @"resumePath": kResumePath });
    SBField *usAuth = Labelled(result.fields, @"Are you legally authorized to work in the United States for any employer?");
    GH_ASSERT(usAuth != nil);
    ...
    NSString *answersJSON = SBJSONString(learned[@"answers"]);
    ...
    SBField *secret = CoreField(@"txt|card", @"Card number", SBKindText);
...
GH_TEST(fixture_greenhouse_form_request_leaves_out_eeo_and_file_paths) {
    ...
    NSData *body = [FixtureCore() formRequestBodyForFieldObjects:[SBField JSONObjectsForFields:result.fields] factKeys:KeysOf(profile)
...
GH_TEST(core_upload_ghost_needs_a_usable_absolute_path) {
    SBField *resume = Upload(@"file|resume", @"Resume/CV", SBUploadKindResume);
    SBField *letter = Upload(@"file|letter", @"Cover Letter", SBUploadKindCoverLetter);
    SBField *inferred = Upload(@"file|cv", @"Upload your CV", nil);          // no uploadKind: read from the label
    SBField *either = Upload(@"file|either", @"Resume or cover letter", nil); // which file is a guess
    SBField *other = Upload(@"file|other", @"Transcript", SBUploadKindOther);
    NSArray<SBField *> *fields = @[ resume, letter, inferred, either, other ];
...
GH_TEST(core_lazy_select_carries_the_intended_answer) {
    SBField *country = LazySelect(@"sel|country", @"Country");
    SBField *heard = LazySelect(@"sel|heard", @"How did you hear about us?");
    SBField *canada = LazySelect(@"sel|auth-ca", @"Are you legally authorized to work in Canada?");
    SBField *graduation = LazySelect(@"sel|grad", @"Expected graduation date");
    SBField *chosen = LazySelect(@"sel|chosen", @"Current country");
    ...
    SBField *known = CoreField(@"sel|known", @"Country", SBKindSelect);
...
GH_TEST(core_work_authorization_answers_another_country_conservatively_and_visibly) {
    ...
    for (NSString *label in [[authorization arrayByAddingObject:sponsorship] arrayByAddingObjectsFromArray:home]) {
        SBField *field = CoreField([NSString stringWithFormat:@"q|%lu", (unsigned long)index++], label, SBKindRadio);
    ...
    SBField *elsewhereSameQuestion = CoreField(@"other-site|auth", authorization[0], SBKindSelect);
...
GH_TEST(core_eeo_questions_are_declined_never_invented) {
    // A protected question with NO way to decline is the one case where Shabang proposes nothing: inventing a
    ...
    for (NSString *label in questions) {
        SBField *field = CoreField([NSString stringWithFormat:@"eeo|%lu", (unsigned long)index++], label, SBKindText);
        [fields addObject:field];
    ...
    SBField *inSection = CoreField(@"eeo|section", @"Please select one", SBKindText);
    ...
    SBField *first = CoreField(@"txt|first", @"First name", SBKindText);
    ...
    // is finished. Shabang picks the form's own wording, never a characteristic.
    SBField *gender = CoreField(@"sel|gender", @"Gender", SBKindSelect);
    gender.options = @[ @{ @"value": @"", @"label": @"Select..." }, @{ @"value": @"m", @"label": @"Male" },
                        @{ @"value": @"f", @"label": @"Female" }, @{ @"value": @"d", @"label": @"Decline To Self Identify" } ];
    SBField *disability = CoreField(@"sel|disability", @"Disability Status", SBKindSelect);
    ...
    // either of them. The screen is not left blank (docs/always-propose.md) -- with nothing to answer, Shabang
    // offers only to take the cursor to the first control, which on the native side is a parked click that
    // Shabang never presses (SBWalkState treats every click ghost as locked).
...
GH_TEST(core_demographic_facts_stay_local_and_a_server_can_never_point_at_one) {
    ...
    SBField *identify = CoreField(@"sel|identify", @"How do you identify?", SBKindSelect);
    identify.options = @[ @{ @"value": @"", @"label": @"Select..." }, @{ @"value": @"man", @"label": @"Man" },
                          @{ @"value": @"woman", @"label": @"Woman" }, @{ @"value": @"nb", @"label": @"Non-binary" } ];
    identify.value = @"";
    SBField *service = CoreField(@"sel|service", @"Have you served?", SBKindSelect);
    ...
    SBField *born = CoreField(@"txt|born", @"When were you born?", SBKindText);
    // A country list with "Isle of Man" is NOT demographic.
    SBField *country = CoreField(@"sel|country", @"Country", SBKindSelect);
    country.options = @[ @{ @"value": @"", @"label": @"Select..." }, @{ @"value": @"IM", @"label": @"Isle of Man" }, @{ @"value": @"CA", @"label": @"Canada" } ];
    country.value = @"";
    NSArray<SBField *> *fields = @[ identify, service, born, country ];
    NSArray *served = @[
    ...
    // tied to "I am not a protected veteran" with confidence -- so Shabang proposes the option that claims the
    ...
    for (SBField *field in @[ identify, service ]) {
    ...
    for (SBField *field in fields) [objects addObject:[field toWireJSONObject]];
...
GH_TEST(field_json_round_trips_upload_kind_and_lazy_options) {
    SBField *field = Upload(@"file|resume", @"Resume/CV", SBUploadKindResume);
    SBField *select = LazySelect(@"sel|country", @"Country");
    ...
    SBField *back = [SBField fieldFromJSONObject:[field toJSONObject]];
    GH_ASSERT_EQUAL_OBJECTS(back.uploadKind, SBUploadKindResume);
    GH_ASSERT([SBField fieldFromJSONObject:[select toJSONObject]].lazyOptions);
    SBField *copy = [select copy];
    GH_ASSERT(copy.lazyOptions);
    GH_ASSERT_EQUAL_OBJECTS(((SBField *)[field copy]).uploadKind, SBUploadKindResume);
    GH_ASSERT_FALSE([SBField fieldFromJSONObject:@{ @"signature": @"s", @"lazyOptions": @"yes" }].lazyOptions);
}
