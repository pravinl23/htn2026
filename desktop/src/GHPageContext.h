// GHPageContext: what a job posting is about, read from the accessibility tree of the page, so drafted answers
...
//
// Pure: a bounded walk over GHAXNode, so it runs on live trees and on saved fixtures alike. Only page text is read;
...
extern const NSUInteger GHPageContextMaxDescription;   // 2000
extern const NSUInteger GHPageContextMaxNodes;         // 6000
/// Wall clock for one walk (main thread): a huge page or a hung app ends it early, `truncated` says so.
extern const NSTimeInterval GHPageContextMaxSeconds;   // 0.25

@interface GHPageContext : NSObject

@property (nonatomic, readonly, copy, nullable) NSString *company;
@property (nonatomic, readonly, copy, nullable) NSString *role;
/// "" when there is none. At most GHPageContextMaxDescription characters.
