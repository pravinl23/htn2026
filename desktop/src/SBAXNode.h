// SBAXNode: a read-only view of one accessibility element. Capture logic only ever talks to this
// protocol, so it runs unchanged against live AXUIElementRefs (SBAXElementNode) and against fake
// trees in tests (SBFakeAXNode).
...
@property (nonatomic, readonly) NSArray<id<SBAXNode>> *children;
@property (nonatomic, readonly, nullable) id<SBAXNode> titleUIElement;
@property (nonatomic, readonly, nullable) id<SBAXNode> parent;
 ...
 * like a twenty-one field form and Shabang offered to fill the other person's messages.
 */
...
- (BOOL)isSameNode:(nullable id<SBAXNode>)other;

@end

/// Seconds an unresponsive app may block one AX call.
extern const float SBAXMessagingTimeoutSeconds; // 0.25

/// A node count and a wall-clock deadline for one bounded walk on the main thread. Every walk outside SBCapture
/// (open panel, combobox list, upload widget checks, page context) spends one of these per visited node, so a huge
/// page cannot freeze Shabang and a hung app (kAXErrorCannotComplete) ends the walk at the first node that says so.
typedef struct {
...
BOOL SBAXWalkBudgetSpend(SBAXWalkBudget *budget, id<SBAXNode> _Nullable node);
...
BOOL SBAXNodeLooksHung(id<SBAXNode> _Nullable node);
...
@interface SBAXElementNode : NSObject <SBAXNode>
...
/// Applies SBAXMessagingTimeoutSeconds process-wide through the system-wide element. SBAccessibility
...
@interface SBFakeAXNode : NSObject <SBAXNode>
...
@property (nonatomic, readwrite, strong, nullable) id<SBAXNode> titleUIElement;
@property (nonatomic, readwrite, weak, nullable) id<SBAXNode> parent;
