...
const NSTimeInterval SBServerRequestTimeout = 3.0;
const NSTimeInterval SBServerStreamTimeout = 30.0;
const NSTimeInterval SBPresenceFreshSeconds = 90.0;
...
static NSString *SBClip(id value, NSUInteger max) {
...
static NSString *SBErrorCode(NSError *error) {
...
@implementation SBFormPrediction
- (NSString *)ghostSource { return self.fromCache ? @"cache" : @"server"; }
@end

@interface SBServerHealth ()
@property (nonatomic, readwrite, copy) NSString *provider;
@property (nonatomic, readwrite) BOOL calibrated;
...
+ (instancetype)presenceFromJSONObject:(id)json now:(NSDate *)now {
    SBPresence *presence = [[SBPresence alloc] init];
...
- (BOOL)isExtensionActiveForBundleId:(NSString *)bundleId {
    NSString *browser = [SBPresence browserNameForBundleId:bundleId];
    NSNumber *age = browser ? self.extensionAges[browser] : nil;
    return age != nil && age.doubleValue < SBPresenceFreshSeconds;
}
...
- (void)finishWithCode:(NSString *)code text:(NSString *)text provider:(NSString *)provider latencyMs:(NSNumber *)latencyMs {
    ...
    self.finished = YES;
    [self.task cancel];
    id<SBGhostTextStreamDelegate> delegate = self.delegate;
...
@interface SBStreamRouter : NSObject <NSURLSessionDataDelegate>
- (void)addStream:(SBGhostTextStream *)stream;
- (NSArray<SBGhostTextStream *> *)allStreams;
@end

@implementation SBStreamRouter {
    NSMutableDictionary<NSNumber *, SBGhostTextStream *> *_streams;
}
...
- (SBGhostTextStream *)streamForTask:(NSURLSessionTask *)task remove:(BOOL)remove {
    @synchronized (self) {
        SBGhostTextStream *stream = _streams[@(task.taskIdentifier)];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)task didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    SBGhostTextStream *stream = [self streamForTask:task remove:NO];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    SBGhostTextStream *stream = [self streamForTask:task remove:YES];
    if (!stream) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        else if (error) code = SBErrorCode(error);
...
- (NSDictionary<NSString *, id> *)entryForOrigin:(NSString *)origin formSignature:(NSString *)formSignature factKeys:(NSArray<NSString *> *)factKeys {
    NSString *key = [SBFormCache keyForOrigin:origin formSignature:formSignature factKeys:factKeys];
    @synchronized (self) {
...
- (void)persist {
    ...
    dispatch_async(_ioQueue, ^{
        ...
        if (data) SBWritePrivateFile(path, data, NULL);
    });
}
...
@interface SBServerClient ()
@property (atomic, readwrite, nullable) NSNumber *lastLatencyMs;
@property (atomic, readwrite, copy, nullable) NSString *lastErrorCode;
@end

@implementation SBServerClient {
    SBCore *_core;
    NSURLSession *_session;
    SBStreamRouter *_router;
    NSString *_baseURLString;
}

- (instancetype)initWithBaseURLString:(NSString *)baseURLString core:(SBCore *)core
                        configuration:(NSURLSessionConfiguration *)configuration cache:(SBFormCache *)cache {
    if (!(self = [super init])) return nil;
    _core = core;
    _cache = cache;
    _baseURLString = [SBServerClient normalizedServerURL:baseURLString ?: @"http://127.0.0.1:8787"];
    NSURLSessionConfiguration *config = [configuration copy] ?: [NSURLSessionConfiguration ephemeralSessionConfiguration];
    config.timeoutIntervalForRequest = SBServerRequestTimeout;
    config.HTTPCookieStorage = nil;
    config.HTTPShouldSetCookies = NO;
    config.URLCache = nil;
    config.requestCachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
    config.URLCredentialStorage = nil;
    _router = [[SBStreamRouter alloc] init];
...
- (void)setBaseURLString:(NSString *)baseURLString {
    @synchronized (self) { _baseURLString = [SBServerClient normalizedServerURL:baseURLString]; }
}
...
- (NSMutableURLRequest *)requestForPath:(NSString *)path body:(NSData *)body accept:(NSString *)accept {
    ...
    request.timeoutInterval = SBServerRequestTimeout;
...
- (void)sendJSON:(NSString *)path body:(NSData *)body completion:(void (^)(NSDictionary *json, NSString *errorCode, double elapsedMs))completion {
    ...
    NSURLSessionDataTask *task = [_session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        SBServerClient *client = weakSelf;
        client.lastErrorCode = code;
        if (!code) client.lastLatencyMs = @(round(elapsed));
        // Numbers and names only: never a body.
        SBLog(@"server: %@ %@ %.0fms", path, code ?: @"ok", elapsed);
        dispatch_async(dispatch_get_main_queue(), ^{ completion(json, code, elapsed); });
    }];
    [task resume];
}

- (void)predictFormForFields:(NSArray<SBField *> *)fields factKeys:(NSArray<NSString *> *)factKeys origin:(NSString *)origin
               formSignature:(NSString *)formSignature completion:(void (^)(SBFormPrediction *, NSString *))completion {
    CFAbsoluteTime started = CFAbsoluteTimeGetCurrent();
    // Wire objects carry no value, and GhostCore.formRequest rebuilds them once more (kinds, limits, sensitivity).
    NSData *body = [_core formRequestBodyForFieldObjects:[SBField wireJSONObjectsForFields:fields] factKeys:factKeys origin:origin formSignature:formSignature];
    if (!body) {
    ...
    if (cached) {
        SBFormPrediction *hit = [[SBFormPrediction alloc] init];
        ...
        if (hit.assignments.count > 0) {
            SBLog(@"server: /v1/predict/form cache=hit assignments=%lu", (unsigned long)hit.assignments.count);
            dispatch_async(dispatch_get_main_queue(), ^{ completion(hit, nil); });
            return;
        }
    }
    SBCore *core = _core;
    SBFormCache *cache = self.cache;
    [self sendJSON:@"/v1/predict/form" body:body completion:^(NSDictionary *json, NSString *errorCode, double elapsedMs) {
        ...
        SBFormPrediction *prediction = [[SBFormPrediction alloc] init];
        prediction.assignments = [core cleanAssignments:json[@"assignments"]];
        prediction.provider = SBClip(json[@"provider"], 40) ?: @"unknown";
        ...
        prediction.fallbackFrom = SBClip(json[@"fallbackFrom"], 40);
...
- (void)checkHealthWithCompletion:(void (^)(SBServerHealth *, NSString *))completion {
    [self sendJSON:@"/v1/health" body:nil completion:^(NSDictionary *json, NSString *errorCode, double elapsedMs) {
        ...
        SBServerHealth *health = [[SBServerHealth alloc] init];
        health.provider = SBClip(json[@"provider"], 40) ?: @"unknown";
        health.calibrated = [json[@"calibrated"] isEqual:@YES];
        health.textProvider = SBClip(json[@"textProvider"], 40) ?: @"unknown";
        health.model = SBClip(json[@"model"], 80);
        health.version = SBClip(json[@"version"], 40);
        completion(health, nil);
    }];
}

- (void)fetchPresenceWithCompletion:(void (^)(SBPresence *, NSString *))completion {
    [self sendJSON:@"/v1/presence" body:nil completion:^(NSDictionary *json, NSString *errorCode, double elapsedMs) {
        if (!json) { completion(nil, errorCode); return; }
        completion([SBPresence presenceFromJSONObject:json now:[NSDate date]], nil);
    }];
}
...
/// a confidence attached to one field is a fingerprint of what Shabang saw on screen.
static NSString *SBConfidenceBucket(double confidence) {
...
static NSString *SBWireAction(NSString *action) {
...
static NSString *SBWireSource(NSString *source) {
...
- (void)reportGhostOutcomeWithAction:(NSString *)action
    ...
    NSDictionary *proposal = @{
        @"index": @1,
        @"action": SBWireAction(action ?: @"click"),
        @"source": SBWireSource(source ?: @"offline"),
        // The desktop ranker is local, so nothing here carries a calibrated probability.
        @"calibrated": @NO,
        @"confidence": SBConfidenceBucket(confidence),
    ...
    [request setValue:@"desktop" forHTTPHeaderField:@"x-shabang-surface"];
    ...
    [[_session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        NSInteger status = [response isKindOfClass:[NSHTTPURLResponse class]] ? ((NSHTTPURLResponse *)response).statusCode : 0;
        if (error || status != 200) {
            SBLog(@"walk-outcome: %@ NOT reported (status=%ld%@)", reported, (long)status, error ? @", transport error" : @"");
            return;
        }
        ...
        SBLog(@"walk-outcome: %@ reported, sentry=%@", reported, captured ? @"captured" : @"not-captured");
    }] resume];
}
...
- (NSData *)ghostTextBodyForLabel:(NSString *)label signature:(NSString *)signature pageContext:(NSDictionary *)pageContext
    ...
    NSString *company = SBClip(pageContext[@"company"], kNameMax), *role = SBClip(pageContext[@"role"], kNameMax);
    NSString *description = SBClip(pageContext[@"description"], kDescriptionMax);
    ...
    for (NSDictionary *item in [_core pastAnswersForProfile:profile label:label]) {
        ...
        NSString *question = SBClip(item[@"question"], kQuestionMax), *answer = SBClip(item[@"answer"], kAnswerMax);
...
- (NSDictionary *)wireConversation:(NSDictionary *)conversation {
    ...
    json[@"messages"] = wire;
    NSString *correspondent = SBClip(conversation[@"correspondent"], kNameMax);
    if (correspondent) json[@"correspondent"] = correspondent;
    return json;
}

- (SBGhostTextStream *)streamGhostTextForFieldLabel:(NSString *)fieldLabel fieldSignature:(NSString *)fieldSignature
                                       ...
                                       conversation:(NSDictionary<NSString *, id> *)conversation
                                           ...
                                           delegate:(id<SBGhostTextStreamDelegate>)delegate {
    SBGhostTextStream *stream = [[SBGhostTextStream alloc] init];
    stream.fieldSignature = fieldSignature ?: @"";
    stream.delegate = delegate;
    stream.received = [NSMutableString string];
    NSString *label = SBClip(fieldLabel, kLabelMax), *signature = SBClip(fieldSignature, kSignatureMax);
    ...
    NSMutableURLRequest *request = body ? [self requestForPath:@"/v1/shabang-text" body:body accept:@"text/event-stream"] : nil;
    ...
    request.timeoutInterval = SBServerStreamTimeout;
    __weak SBGhostTextStream *weakStream = stream;
    stream.parser = [[SBSSEParser alloc] initWithHandler:^(NSString *data) { [weakStream handleEventData:data]; }];
    stream.task = [_session dataTaskWithRequest:request];
    [_router addStream:stream];
    SBLog(@"server: /v1/shabang-text started label=%@", SBLogLabel(label));
    [stream.task resume];
    return stream;
}

- (void)cancelAll {
    for (SBGhostTextStream *stream in [_router allStreams]) [stream cancel];
