// GHScanSources: what cold start could read, counted without reading anything (docs/cold-start.md sections 2 and 4).
//
...
//
// Paths collected here are candidates for GHColdStart to open later. They are personal data: they are never
// logged and never written into a report (see GHColdStart's value-free report).
#import <Foundation/Foundation.h>

NS_ASSUME_NONNULL_BEGIN

/// Mirrors PermissionState in shared/src/coldstart/plan.ts.
typedef NS_ENUM(NSInteger, GHPermissionState) {
    GHPermissionNotRequired = 0,   // Spotlight metadata: no dialog exists for it
    GHPermissionGranted = 1,
    GHPermissionMissing = 2,
    GHPermissionUnknown = 3,       // the capability check has not run (treated as missing when planning)
};
...
extern NSString *const GHScanKindSpotlight;
...
extern NSString *const GHScanKindDock;
extern NSString *const GHScanKindLoginItems;
extern NSString *const GHScanKindRecentApps;
extern NSString *const GHScanKindRecentDocs;
extern NSString *const GHScanKindAppInventory;
extern NSString *const GHScanKindContacts;
extern NSString *const GHScanKindResume;
extern NSString *const GHScanKindBrowserHistory;
extern NSString *const GHScanKindCalendar;
extern NSString *const GHScanKindMail;
extern NSString *const GHScanKindProjects;

/// One row of the consent panel, before the shared planner turns it into a plan.
@interface GHScanSource : NSObject

@property (nonatomic, copy, readonly) NSString *kind;
/// What a COUNT found. Negative means "not counted" (the planner then plans for the cap).
@property (nonatomic) NSInteger itemCount;
@property (nonatomic) GHPermissionState permission;
/// A reason CODE for the report ("not-implemented", "no-profile", "protected-root"), never a path or a value.
@property (nonatomic, copy, nullable) NSString *detail;
/// YES when Ghost has no reader for this source yet, whatever the permission says.
@property (nonatomic) BOOL unavailable;
/// Candidate files for GHColdStart. Personal data: never logged, never reported.
...
@protocol GHScanEnvironment <NSObject>
...
@interface GHScanEnvironmentMac : NSObject <GHScanEnvironment>
...
@interface GHScanSources : NSObject

- (instancetype)initWithEnvironment:(id<GHScanEnvironment>)environment NS_DESIGNATED_INITIALIZER;
- (instancetype)init NS_UNAVAILABLE;

@property (nonatomic, readonly, strong) id<GHScanEnvironment> environment;
