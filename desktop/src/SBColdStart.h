// SBColdStart: the one-time local scan that gives Shabang a knowledge graph on day one (docs/cold-start.md).
//
// SBScanSources counts what is there; this runs the plan over the sources the user switched on, hands what it
...
//   value-free   the report SBColdStart hands out carries labels, categories, confidences, provenance KINDS and
...
@class SBCore, SBProfileStore;

NS_ASSUME_NONNULL_BEGIN

/// Why a run ended.
typedef NS_ENUM(NSInteger, SBColdStartStop) {
    SBColdStartStopFinished = 0,
    SBColdStartStopBudget = 1,      // the wall clock or the file cap ran out; what was found is still returned
    SBColdStartStopCancelled = 2,
    SBColdStartStopNoCore = 3,      // shabang-core.js is missing: nothing is read at all
};
...
/// A read-only COPY of a database, so the browser's own file is never opened by Shabang. Returns the copy's path.
...
@interface SBColdStartFilesMac : NSObject <SBColdStartFiles>
...
@interface SBColdStartProposal : NSObject
...
/// What one run found.
@interface SBColdStartResult : NSObject
@property (nonatomic, copy) NSString *scanIdentifier;
@property (nonatomic, copy) NSArray<SBColdStartProposal *> *proposals;
...
@property (nonatomic) SBColdStartStop stop;
...
BOOL SBColdStartIsValueFree(id object, NSString *_Nullable *_Nullable offender);

@interface SBColdStart : NSObject

- (instancetype)initWithCore:(nullable SBCore *)core files:(id<SBColdStartFiles>)files NS_DESIGNATED_INITIALIZER;
- (instancetype)init NS_UNAVAILABLE;

@property (nonatomic) SBColdStartBudget budget;
@property (nonatomic, strong, nullable) id<SBColdStartClock> clock;
...
@property (nonatomic, strong, nullable) id<SBScanEnvironment> environment;
...
- (nullable NSDictionary<NSString *, id> *)planForSources:(NSArray<SBScanSource *> *)sources
...
- (void)runSources:(NSArray<SBScanSource *> *)sources
      enabledKinds:(NSSet<NSString *> *)enabledKinds
        completion:(void (^)(SBColdStartResult *result))completion;
...
typedef struct {
    NSUInteger applied;
    NSUInteger conflicts;     // the profile already holds a different value; the user decides, not Shabang
    NSUInteger unchanged;     // already exactly this value
    NSUInteger ignored;       // not accepted in the report
} SBColdStartApplyCounts;
...
SBColdStartApplyCounts SBColdStartApply(NSDictionary *report, NSDictionary *pending, SBProfileStore *store, NSError *_Nullable *_Nullable error);

/// Seeds memory.json with the habit priors, but ONLY when there is no memory yet: a cold start may give Shabang
...
/// What Shabang knows: counts per screen kind, per source, the file size and the most used surfaces. The surface
...
BOOL SBColdStartForgetSource(SBCore *core, NSString *graphPath, NSString *kind,
