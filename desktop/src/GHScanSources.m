...
NSString *const GHScanKindSpotlight = @"spotlight";
NSString *const GHScanKindDock = @"dock";
NSString *const GHScanKindLoginItems = @"login-items";
NSString *const GHScanKindRecentApps = @"recent-apps";
NSString *const GHScanKindRecentDocs = @"recent-docs";
NSString *const GHScanKindAppInventory = @"app-inventory";
NSString *const GHScanKindContacts = @"contacts";
NSString *const GHScanKindResume = @"resume";
NSString *const GHScanKindBrowserHistory = @"browser-history";
NSString *const GHScanKindCalendar = @"calendar";
NSString *const GHScanKindMail = @"mail";
NSString *const GHScanKindProjects = @"projects";

NSString *GHPermissionStateName(GHPermissionState state) {
    switch (state) {
        case GHPermissionNotRequired: return @"not-required";
        case GHPermissionGranted: return @"granted";
        case GHPermissionMissing: return @"missing";
        case GHPermissionUnknown: break;
    }
    return @"unknown";
}

#pragma mark - GHScanSource

@implementation GHScanSource

+ (instancetype)sourceWithKind:(NSString *)kind {
    GHScanSource *source = [[GHScanSource alloc] init];
    source->_kind = [kind copy];
    source.itemCount = -1;
    source.permission = GHPermissionUnknown;
    source.paths = @[];
    return source;
}
...
- (NSDictionary<NSString *, id> *)descriptorEnabled:(BOOL)enabled {
    ...
    descriptor[@"permission"] = GHPermissionStateName(self.permission);
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<GHScanSource %@ count=%ld permission=%@ files=%lu%@>", self.kind, (long)self.itemCount,
                                      GHPermissionStateName(self.permission), (unsigned long)self.paths.count,
...
static NSArray<NSString *> *GHProtectedRelativeRoots(void) {
...
@implementation GHScanEnvironmentMac
...
+ (BOOL)isProtectedPath:(NSString *)path home:(NSString *)home {
    ...
    for (NSString *relative in GHProtectedRelativeRoots()) {
...
- (BOOL)isReadableFileAtPath:(NSString *)path {
    ...
    if ([GHScanEnvironmentMac isProtectedPath:path home:self.homeDirectory]) return NO;
...
- (nullable NSString *)runTool:(NSString *)tool arguments:(NSArray<NSString *> *)arguments {
    ...
    if (![task launchAndReturnError:&error]) {
        GHLog(@"scan: %@ did not start (%@)", tool.lastPathComponent, error.localizedDescription ?: @"?");
        return nil;
    }
    ...
    if (dispatch_semaphore_wait(done, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.subprocessTimeout * NSEC_PER_SEC))) != 0) {
        [task terminate];
        GHLog(@"scan: %@ timed out after %.0f s", tool.lastPathComponent, self.subprocessTimeout);
        return nil;
    }
    [task waitUntilExit];
...
- (NSArray<NSString *> *)pathsForQueryArguments:(NSArray<NSString *> *)arguments limit:(NSUInteger)limit {
    ...
    for (NSString *line in [text componentsSeparatedByString:@"\n"]) {
        ...
        if ([GHScanEnvironmentMac isProtectedPath:line home:self.homeDirectory]) continue;
...
- (GHPermissionState)authorizationStatusForKind:(NSString *)kind {
    ...
    NSString *className = [kind isEqualToString:GHScanKindContacts] ? @"CNContactStore"
                        : [kind isEqualToString:GHScanKindCalendar] ? @"EKEventStore" : nil;
    Class store = className ? NSClassFromString(className) : Nil;
    if (!store) return GHPermissionUnknown;
    SEL selector = NSSelectorFromString(@"authorizationStatusForEntityType:");
    if (![store respondsToSelector:selector]) return GHPermissionUnknown;
    NSMethodSignature *signature = [store methodSignatureForSelector:selector];
    if (!signature) return GHPermissionUnknown;
    ...
    // 3 = authorized, 4 = limited (Contacts, macOS 14+). Everything else means Ghost may not read it.
    return (status == 3 || status == 4) ? GHPermissionGranted : GHPermissionMissing;
}
...
- (NSArray<NSString *> *)entryNamesAtDirectoryPath:(NSString *)path {
    ...
    if ([GHScanEnvironmentMac isProtectedPath:path home:self.homeDirectory]) return @[];
...
#pragma mark - GHScanSources

@implementation GHScanSources

- (instancetype)initWithEnvironment:(id<GHScanEnvironment>)environment {
    if ((self = [super init])) {
...
- (NSDictionary<NSString *, NSNumber *> *)neverReadCounts {
    ...
    [[GHScanSources neverReadQueries] enumerateKeysAndObjectsUsingBlock:^(NSString *reason, NSString *query, BOOL *stop) {
...
- (GHScanSource *)spotlightSourceWithDocuments:(NSInteger)documents
                                         cards:(NSInteger)cards
                                        events:(NSInteger)events
                                     manifests:(NSInteger)manifests {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindSpotlight];
    source.permission = GHPermissionNotRequired;
...
- (GHScanSource *)dockSource {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindDock];
    NSString *path = [[self.environment homeDirectory] stringByAppendingPathComponent:[GHScanSources dockRelativePath]];
    source.itemCount = -1;
    if ([self.environment isReadableFileAtPath:path]) {
        source.permission = GHPermissionNotRequired;
        source.paths = @[ path ];
    } else {
        source.permission = GHPermissionMissing;
...
- (GHScanSource *)loginItemsSource {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindLoginItems];
    NSString *directory = [[self.environment homeDirectory] stringByAppendingPathComponent:[GHScanSources loginItemsRelativeDirectory]];
...
- (GHScanSource *)recentAppsSourceWithCount:(NSInteger)count {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindRecentApps];
    source.permission = GHPermissionNotRequired;
...
- (GHScanSource *)recentDocsSourceWithCount:(NSInteger)count {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindRecentDocs];
    source.permission = GHPermissionNotRequired;
    source.itemCount = count >= 0 ? count : -1;
    if (count < 0) source.detail = @"spotlight-unavailable";
    return source;
...
- (GHScanSource *)appInventorySource {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindAppInventory];
    source.permission = GHPermissionNotRequired;
    NSMutableArray<NSString *> *roots = [NSMutableArray array];
    NSInteger count = 0;
    for (NSString *root in [GHScanSources applicationRootsForHome:[self.environment homeDirectory]]) {
        NSInteger here = 0;
...
- (GHScanSource *)contactsSourceWithCards:(NSArray<NSString *> *)cards {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindContacts];
    source.permission = [self.environment authorizationStatusForKind:GHScanKindContacts];
    ...
    if (source.permission == GHPermissionGranted) {
...
- (GHScanSource *)resumeSourceWithCount:(NSInteger)count {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindResume];
    source.itemCount = MAX(count, 0);
    NSArray<NSString *> *candidates = count == 0 ? @[] : [self.environment pathsForSpotlightQuery:[GHScanSources queryForResumeDocuments]
...
- (GHScanSource *)browserHistorySource {
...
- (GHScanSource *)calendarSourceWithCount:(NSInteger)count {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindCalendar];
    source.permission = [self.environment authorizationStatusForKind:GHScanKindCalendar];
    source.itemCount = count >= 0 ? count : -1;
...
- (GHScanSource *)mailSource {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindMail];
...
- (GHScanSource *)projectsSourceWithCount:(NSInteger)count {
    GHScanSource *source = [GHScanSource sourceWithKind:GHScanKindProjects];
    source.itemCount = MAX(count, 0);
    NSArray<NSString *> *manifests = count == 0 ? @[] : [self.environment pathsForSpotlightQuery:[GHScanSources queryForProjectManifests]
                                                                                           limit:self.maxPathsPerSource];
    NSMutableArray<NSString *> *readable = [NSMutableArray array];
...
- (NSArray<GHScanSource *> *)discover {
    NSInteger resumeCount = [self.environment countForSpotlightQuery:[GHScanSources queryForResumeDocuments]];
    NSInteger cardCount = [self.environment countForSpotlightQuery:[GHScanSources queryForVCards]];
    NSInteger eventCount = [self.environment countForSpotlightQuery:[GHScanSources queryForCalendarFiles]];
    NSInteger projectCount = [self.environment countForSpotlightQuery:[GHScanSources queryForProjectManifests]];
    NSArray<NSString *> *cards = cardCount <= 0 ? @[] : [self.environment pathsForSpotlightQuery:[GHScanSources queryForVCards] limit:self.maxPathsPerSource];
    NSInteger recentApps = [self.environment countForSystemSpotlightQuery:[GHScanSources queryForRecentApplications]];
