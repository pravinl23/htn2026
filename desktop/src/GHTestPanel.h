// GHTestPanel: two buttons floating over everything, for telling apart the two ways an accept can fail.
//
// When a ghost does not take, there are two completely different causes and they look identical from the
// outside:
//
//   1. the key never reached Ghost at all (the app owns Tab, or focus is somewhere Ghost will not take it from);
//   2. the key reached Ghost and the actuation did nothing (the control does not implement AXPress, the
//      element moved, the app ignored the click).
//
// So: press a button, keep your hands off the keyboard, and one second later Ghost does the thing by itself.
//
...
//   Accept   goes straight to the accept path, exactly as the Ghost key does. If this does nothing either,
...
extern const NSTimeInterval GHTestPanelDelay;   // 1.0

@interface GHTestPanel : NSObject

- (instancetype)init NS_UNAVAILABLE;
- (instancetype)initWithController:(GHController *)controller NS_DESIGNATED_INITIALIZER;
