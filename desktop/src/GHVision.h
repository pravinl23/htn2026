// GHVision: naming what has no name (docs/anywhere.md section 4, docs/server-api.md `/v1/vision/label`).
//
// After the heuristics, a control can still be `unknown` with nothing readable on it: an icon-only button in a
// canvas app, a custom-drawn widget, a player's glyph with no identifier. Ghost then crops THOSE CONTROLS ONLY
...
//   - never for a window with a sensitive field on screen (GHPageSignals.sensitiveOnScreen), at all;
...
//
// Screen Recording: taking a screenshot needs the macOS Screen Recording permission, which Ghost may not have.
...
extern NSString *const GHVisionReasonScreenRecording;  // "needs Screen Recording"
extern NSString *const GHVisionReasonSensitive;        // a sensitive field is on screen: nothing is captured
extern NSString *const GHVisionReasonNothingToName;    // every control already has a name and a role
extern NSString *const GHVisionReasonCaptureFailed;    // the screenshot API returned nothing

/// One control to name: where it is on screen, and which captured field it belongs to.
@interface GHVisionBox : NSObject
...
@interface GHVision : NSObject

- (instancetype)initWithBaseURLString:(nullable NSString *)baseURLString NS_DESIGNATED_INITIALIZER;
- (instancetype)init NS_UNAVAILABLE;

/// Ghost may take a screenshot right now (the Screen Recording permission is granted), or a `screenshot`
...
+ (nullable NSDictionary<NSString *, id> *)requestBodyForStrip:(NSData *)png
                                                          size:(CGSize)size
                                                         boxes:(NSArray<GHVisionBox *> *)boxes
