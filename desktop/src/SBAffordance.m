...
/// How far up from a control Shabang looks for that shared ancestor.
...
static NSString *SBAffSquash(NSString *_Nullable text) {
...
static BOOL SBAffMatches(NSRegularExpression *pattern, NSString *_Nullable text) {
    if (text.length == 0) return NO;
    return [pattern numberOfMatchesInString:text options:0 range:NSMakeRange(0, text.length)] > 0;
}

static NSRegularExpression *SBAffPattern(NSString *source) {
 ...
 * and not one of them classified as anything -- so the best Shabang could offer was a per-video overflow menu.
 *
 * "ago" is excluded, because "5 minutes ago" is when a thing happened, not how long it runs.
 */
static NSRegularExpression *SBAffSpokenDurationPattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"\\b\\d{1,3} (hour|hours|minute|minutes|second|seconds)\\b(?!,? ago)"); });
    return pattern;
}

/// "0:42", "12:03", "1:03:11": what a player draws beside its scrubber, in every language.
static NSRegularExpression *SBAffDurationPattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"^-?\\d{1,3}:[0-5]\\d(:[0-5]\\d)?$"); });
    return pattern;
}
...
static NSRegularExpression *SBAffPricePattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        pattern = SBAffPattern(@"[$£€¥₹₩₽₺₪₫฿¢]\\s?\\d|\\d\\s?[$£€¥₹₩₽₺₪₫฿¢]|"
...
static NSRegularExpression *SBAffScrubPattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"\\b(seek|scrub|playhead|elapsed|remaining|time|timeline|progress|position|duration)\\b"); });
    return pattern;
}

/// A role description that says "this node IS a media element". Only ever read off the node's own role words.
static NSRegularExpression *SBAffMediaPattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"\\b(video|audio|movie|media player|player)\\b"); });
    return pattern;
}

/// The one control every player has, and the thing a window is FOR when it has it.
static NSRegularExpression *SBAffTransportPattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"\\b(play|pause)\\b"); });
    return pattern;
}
...
static NSRegularExpression *SBAffTransportMatePattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"\\b(un)?mute\\b|\\bvolume\\b|\\bfull ?screen\\b|\\bcaptions?\\b|\\bsubtitles?\\b|\\bmini ?player\\b|\\bautoplay\\b|\\bauto-play\\b|\\bnext (video|track|song|episode)\\b|\\bplayback (speed|rate)\\b"); });
    return pattern;
}

/// The small count an icon carries: "3", "12", "7 new", "2 items". Never a year, a price or an id.
static NSRegularExpression *SBAffBadgePattern(void) {
    static NSRegularExpression *pattern;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ pattern = SBAffPattern(@"^\\(?(\\d{1,3})\\)?(\\s*(new|unread|items?|messages?|notifications?))?$"); });
    return pattern;
}

static uint32_t SBAffHash(NSString *text) {
...
#pragma mark - SBPageSignals

@implementation SBPageSignals

- (id)copyWithZone:(NSZone *)zone {
    SBPageSignals *copy = [[SBPageSignals allocWithZone:zone] init];
...
@implementation SBAffEntry
@end

@implementation SBAffordance

+ (BOOL)looksLikeDuration:(NSString *)text {
    return SBAffMatches(SBAffDurationPattern(), SBAffSquash(text));
}

+ (BOOL)namesADuration:(NSString *)text {
    NSString *clean = SBAffSquash(text);
    return SBAffMatches(SBAffDurationPattern(), clean) || SBAffMatches(SBAffSpokenDurationPattern(), clean);
}

+ (BOOL)looksLikePrice:(NSString *)text {
    NSString *clean = SBAffSquash(text);
    if (clean.length == 0 || clean.length > 40) return NO;
    return SBAffMatches(SBAffPricePattern(), clean);
}

+ (NSUInteger)countInBadgeText:(NSString *)text {
    NSString *clean = SBAffSquash(text);
    if (clean.length == 0 || clean.length > 20) return 0;
    NSTextCheckingResult *match = [SBAffBadgePattern() firstMatchInString:clean options:0 range:NSMakeRange(0, clean.length)];
...
+ (SBPageSignals *)annotateResult:(SBCaptureResult *)result window:(id<SBAXNode>)window {
    return [self annotateResult:result window:window maxNodes:kDefaultMaxNodes timeBudget:kDefaultTimeBudget];
}

+ (SBPageSignals *)annotateResult:(SBCaptureResult *)result
                           window:(id<SBAXNode>)window
    ...
    NSMutableArray<SBAffEntry *> *entries = [NSMutableArray array];
    NSMutableArray<NSMutableArray<NSNumber *> *> *children = [NSMutableArray array];
    SBAXWalkBudget budget = SBAXWalkBudgetMake(maxNodes ?: kDefaultMaxNodes, timeBudget > 0 ? timeBudget : kDefaultTimeBudget);
    double textChars = 0;
    ...
    for (SBAffEntry *entry in entries) {
    ...
    if (!signals.hasMediaElement) {
        for (SBAffEntry *entry in entries) if (entry.mediaSeed) { signals.hasMediaElement = YES; break; }
    }
    ...
    for (SBField *field in fields) if (!field.unnamed) controls++;
...
+ (void)visit:(id<SBAXNode>)node
       parent:(NSInteger)parent
        depth:(NSUInteger)depth
      entries:(NSMutableArray<SBAffEntry *> *)entries
     children:(NSMutableArray<NSMutableArray<NSNumber *> *> *)children
       budget:(SBAXWalkBudget *)budget
    textChars:(double *)textChars {
    if (!SBAXWalkBudgetSpend(budget, node)) return;
    SBAffEntry *entry = [[SBAffEntry alloc] init];
    entry.node = node;
    entry.parent = parent;
    entry.depth = depth;
    ...
    if ([node.role isEqualToString:kRoleStaticText]) {
        NSString *text = SBAffSquash(node.value ?: node.title);
...
+ (NSString *)shapeOf:(id<SBAXNode>)node {
...
+ (BOOL)isMediaElement:(id<SBAXNode>)node {
    ...
    return SBAffMatches(SBAffMediaPattern(), SBAffSquash(node.roleDescription));
}
 ...
 * the media priors never applied, and the best thing Shabang could find to offer after opening a video was an
 ...
 */
+ (BOOL)fieldsLookLikeAPlayer:(NSArray<SBField *> *)fields {
    BOOL transport = NO;
    NSUInteger mates = 0;
    for (SBField *field in fields) {
        NSString *name = SBAffSquash(field.label);
        if (name.length == 0) continue;
        if (SBAffMatches(SBAffTransportPattern(), name)) transport = YES;
        if (SBAffMatches(SBAffTransportMatePattern(), name)) mates++;
    }
    return transport && mates > 0;
}

+ (void)markScrubbersIn:(NSArray<SBAffEntry *> *)entries children:(NSArray<NSMutableArray<NSNumber *> *> *)children {
    for (NSUInteger i = 0; i < entries.count; i++) {
        SBAffEntry *entry = entries[i];
        if (entry.mediaSeed || ![entry.node.role isEqualToString:kRoleSlider]) continue;
        id<SBAXNode> node = entry.node;
        NSString *naming = SBAffSquash([@[node.title ?: @"", node.axDescription ?: @"", node.roleDescription ?: @"", node.identifier ?: @""] componentsJoinedByString:@" "]);
        BOOL named = SBAffMatches(SBAffScrubPattern(), naming);
        ...
        if (!named && !timed && entry.parent >= 0) {
            for (NSNumber *sibling in children[(NSUInteger)entry.parent]) {
                SBAffEntry *other = entries[sibling.unsignedIntegerValue];
 ...
 */
static BOOL SBAffGroupIsAToolbar(NSArray<NSNumber *> *group, NSArray<SBAffEntry *> *entries) {
...
+ (void)findListsIn:(NSArray<SBAffEntry *> *)entries children:(NSArray<NSMutableArray<NSNumber *> *> *)children {
    for (NSUInteger i = 0; i < entries.count; i++) {
        ...
        for (NSString *shape in byShape) {
            ...
            // place.
            if (SBAffGroupIsAToolbar(group, entries)) continue;
            // The signature names the STRUCTURE, never the content: the same grid keeps its key across reloads.
            NSString *signature = [NSString stringWithFormat:@"l%08x", SBAffHash([NSString stringWithFormat:@"%@#%@", entries[i].shape ?: @"", shape])];
            NSUInteger index = 0;
            for (NSNumber *kid in group) {
                SBAffEntry *member = entries[kid.unsignedIntegerValue];
...
+ (NSString *)mainListSignatureIn:(NSArray<SBAffEntry *> *)entries repeats:(NSUInteger *)repeats {
...
+ (void)annotateFields:(NSArray<SBField *> *)fields
                result:(SBCaptureResult *)result
               entries:(NSArray<SBAffEntry *> *)entries
    ...
    for (SBField *field in fields) {
        id<SBAXNode> node = [result nodeForSignature:field.signature];
        ...
        field.namesDuration = [SBAffordance namesADuration:field.label];
        ...
        for (NSInteger up = index, lift = 0; up >= 0 && lift <= (NSInteger)ceiling; up = entries[(NSUInteger)up].parent, lift++) {
            SBAffEntry *entry = entries[(NSUInteger)up];
...
+ (void)lift:(NSUInteger)index by:(NSUInteger)lift entries:(NSArray<SBAffEntry *> *)entries into:(NSMutableIndexSet *)set {
...
+ (NSUInteger)badgeUnder:(NSUInteger)index entries:(NSArray<SBAffEntry *> *)entries children:(NSArray<NSMutableArray<NSNumber *> *> *)children {
    id<SBAXNode> node = entries[index].node;
    ...
    for (NSUInteger depth = 0; depth < 2 && frontier.count > 0; depth++) {
        ...
        for (NSNumber *at in frontier) {
            for (NSNumber *kid in children[at.unsignedIntegerValue]) {
                SBAffEntry *child = entries[kid.unsignedIntegerValue];
...
+ (NSInteger)indexOfNode:(id<SBAXNode>)node in:(NSArray<SBAffEntry *> *)entries {
    if (!node) return -1;
    for (NSUInteger i = 0; i < entries.count; i++) {
