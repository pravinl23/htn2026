// SBServerClient: the only place Shabang Desktop talks to the local prediction server (default
...
//   - POST /v1/shabang-text carries only GhostCore.textFacts (no contact details) and is refused locally for
//     sensitive labels.
...
@class SBCore, SBGhostTextStream;

NS_ASSUME_NONNULL_BEGIN

extern const NSTimeInterval SBServerRequestTimeout;      // 3 s
extern const NSTimeInterval SBServerStreamTimeout;       // 30 s
extern const NSTimeInterval SBPresenceFreshSeconds;      // 90 s

#pragma mark - results

@interface SBFormPrediction : NSObject
...
/// "cache" or "server": the GhostSource to hand to SBCore.
@property (nonatomic, readonly) NSString *ghostSource;
@end

@interface SBServerHealth : NSObject
@property (nonatomic, readonly, copy) NSString *provider;
@property (nonatomic, readonly) BOOL calibrated;
...
@interface SBSSEParser : NSObject
...
@protocol SBGhostTextStreamDelegate <NSObject>
- (void)ghostTextStream:(SBGhostTextStream *)stream didReceiveDelta:(NSString *)delta;
- (void)ghostTextStream:(SBGhostTextStream *)stream didFinishWithText:(NSString *)text provider:(NSString *)provider latencyMs:(nullable NSNumber *)latencyMs;
...
@interface SBGhostTextStream : NSObject
...
@interface SBFormCache : NSObject
...
@interface SBServerClient : NSObject
...
- (instancetype)initWithBaseURLString:(nullable NSString *)baseURLString
                        ...
                        configuration:(nullable NSURLSessionConfiguration *)configuration
                                cache:(nullable SBFormCache *)cache NS_DESIGNATED_INITIALIZER;
- (instancetype)init NS_UNAVAILABLE;
...
@property (nonatomic, readonly, nullable) SBFormCache *cache;
...
- (void)predictFormForFields:(NSArray<SBField *> *)fields
                    factKeys:(NSArray<NSString *> *)factKeys
                      origin:(NSString *)origin
               formSignature:(NSString *)formSignature
                  completion:(void (^)(SBFormPrediction *_Nullable prediction, NSString *_Nullable errorCode))completion;

- (void)checkHealthWithCompletion:(void (^)(SBServerHealth *_Nullable health, NSString *_Nullable errorCode))completion;
- (void)fetchPresenceWithCompletion:(void (^)(SBPresence *_Nullable presence, NSString *_Nullable errorCode))completion;
...
- (nullable SBGhostTextStream *)streamGhostTextForFieldLabel:(NSString *)fieldLabel
