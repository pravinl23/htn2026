...
#import "GHPageContext.h"   // normalizedText / text:cappedAt: -- the same trimming every context uses

const NSUInteger GHConversationMaxMessages = 14;
const NSUInteger GHConversationMaxMessageChars = 400;
const NSUInteger GHConversationMaxTotalChars = 2000;
const NSUInteger GHConversationMaxNodes = 4000;
const NSTimeInterval GHConversationMaxSeconds = 0.20;
...
+ (instancetype)messageFrom:(NSString *)from text:(NSString *)text fromMe:(BOOL)fromMe {
    GHMessage *message = [[self alloc] init];
    message.from = from ?: @"";
    message.text = text ?: @"";
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<GHMessage from=%lu chars=%lu mine=%d>", (unsigned long)self.from.length,
            (unsigned long)self.text.length, self.fromMe];
}

@end

#pragma mark - GHConversation

@interface GHConversation ()
@property (nonatomic, readwrite, copy) NSArray<GHMessage *> *messages;
...
static BOOL GHLooksLikeATime(NSString *text) {
...
@implementation GHConversation

+ (instancetype)conversationFromNode:(id<GHAXNode>)root {
    return [self conversationFromNode:root maxNodes:GHConversationMaxNodes];
}
...
+ (instancetype)conversationFromNode:(id<GHAXNode>)root maxNodes:(NSUInteger)maxNodes column:(CGRect)column {
    ...
    GHAXWalkBudget budget = GHAXWalkBudgetMake(maxNodes, GHConversationMaxSeconds);
    ...
    NSArray<GHMessage *> *kept = [self lastMessages:found];
    conversation.messages = kept;
    for (GHMessage *message in kept.reverseObjectEnumerator) {
...
+ (NSArray<GHMessage *> *)lastMessages:(NSArray<GHMessage *> *)all {
    NSMutableArray<GHMessage *> *kept = [NSMutableArray array];
    NSUInteger total = 0;
    for (GHMessage *message in all.reverseObjectEnumerator) {
        if (kept.count >= GHConversationMaxMessages) break;
        if (total + message.text.length > GHConversationMaxTotalChars && kept.count > 0) break;
...
+ (void)collect:(id<GHAXNode>)node
    ...
    while (head < queue.count) {
        id<GHAXNode> current = queue[head++];
        if (!GHAXWalkBudgetSpend(budget, current)) return;
        GHMessage *message = [self messageFromDescription:current.axDescription];
        if (message) {
        ...
        for (id<GHAXNode> child in current.children) [queue addObject:child];
    }
}
 ...
 */
+ (CGRect)bubbleFrameOf:(id<GHAXNode>)row {
    CGRect bubbles = CGRectNull;
    CGRect narrowest = CGRectNull;
    NSMutableArray<id<GHAXNode>> *queue = [NSMutableArray arrayWithObject:row];
    NSUInteger head = 0, visited = 0;
    while (head < queue.count && visited < kBubbleSearchNodes) {
        id<GHAXNode> node = queue[head++];
        visited++;
        ...
        for (id<GHAXNode> child in node.children) [queue addObject:child];
    }
...
+ (GHMessage *)messageFromDescription:(NSString *)description {
    NSString *text = [GHPageContext normalizedText:description];
    if (text.length == 0) return nil;
    NSArray<NSString *> *parts = [text componentsSeparatedByString:@","];
    if (parts.count < 3) return nil;
    if (!GHLooksLikeATime(parts.lastObject)) return nil;   // no timestamp, not a message
    ...
    said = [GHPageContext normalizedText:said];
    if (said.length == 0) return nil;

    if (from.length > kMaxNameChars) from = [GHPageContext text:from cappedAt:kMaxNameChars];
    if (said.length > GHConversationMaxMessageChars) said = [GHPageContext text:said cappedAt:GHConversationMaxMessageChars];
    return [GHMessage messageFrom:from text:said fromMe:NO];
}
...
- (NSDictionary<NSString *, id> *)dictionary {
    ...
    for (GHMessage *message in self.messages) {
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<GHConversation %lu messages, %lu nodes%@>", (unsigned long)self.messages.count,
