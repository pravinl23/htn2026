...
const CGFloat SBVisibleEnoughFraction = 0.6;

BOOL SBRectIsUsable(CGRect rect) {
...
CGFloat SBRectArea(CGRect rect) {
    return SBRectIsUsable(rect) ? rect.size.width * rect.size.height : 0;
}

CGRect SBRectFlip(CGRect rect, CGFloat primaryHeight) {
    return CGRectMake(rect.origin.x, primaryHeight - rect.origin.y - rect.size.height, rect.size.width, rect.size.height);
}

CGRect SBRectPixelAligned(CGRect rect, CGFloat scale) {
...
CGRect SBRectClipped(CGRect rect, CGRect clip) {
    if (CGRectIsNull(clip)) return rect;
    return CGRectIntersection(rect, clip);
}

CGFloat SBVisibleFraction(CGRect field, CGRect window, CGRect screen) {
    CGFloat whole = SBRectArea(field);
    if (whole <= 0) return 0;
    CGRect seen = CGRectIntersection(SBRectClipped(field, window), screen);
    return SBRectArea(seen) / whole;
}

@implementation SBScreenLayout {
...
- (CGRect)axFrameAtIndex:(NSUInteger)index {
    return index < _frames.count ? SBRectFlip(_frames[index].rectValue, self.primaryHeight) : CGRectNull;
}
...
- (CGRect)appKitRectFromAXRect:(CGRect)axRect { return SBRectFlip(axRect, self.primaryHeight); }
- (CGRect)axRectFromAppKitRect:(CGRect)appKitRect { return SBRectFlip(appKitRect, self.primaryHeight); }

- (NSUInteger)screenIndexForAXRect:(CGRect)axRect {
    if (!SBRectIsUsable(axRect)) return NSNotFound;
    NSUInteger best = NSNotFound;
    CGFloat bestArea = 0;
    for (NSUInteger i = 0; i < _frames.count; i++) {
        CGFloat area = SBRectArea(CGRectIntersection(axRect, [self axFrameAtIndex:i]));
        if (area > bestArea) {
            bestArea = area;
            best = i;
        }
...
- (CGRect)localRectFromAXRect:(CGRect)axRect screen:(NSUInteger)index {
    ...
    return SBRectPixelAligned(local, [self scaleAtIndex:index]);
}
...
- (CGRect)visiblePartOfAXRect:(CGRect)axRect inWindow:(CGRect)windowAXFrame screen:(NSUInteger)index {
    if (index >= _frames.count || !SBRectIsUsable(axRect)) return CGRectNull;
    return CGRectIntersection(SBRectClipped(axRect, windowAXFrame), [self axFrameAtIndex:index]);
}
...
- (CGFloat)visibleFractionOfAXRect:(CGRect)axRect inWindow:(CGRect)windowAXFrame {
    ...
    return SBVisibleFraction(axRect, windowAXFrame, [self axFrameAtIndex:index]);
}

- (BOOL)isAXRectVisibleEnough:(CGRect)axRect inWindow:(CGRect)windowAXFrame {
    return [self visibleFractionOfAXRect:axRect inWindow:windowAXFrame] >= SBVisibleEnoughFraction;
}
