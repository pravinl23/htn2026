// SBKeyPoster: the one way the open-panel and combobox drivers touch the keyboard (docs/desktop-realworld.md,
...
//   - Every event is tagged with SBSyntheticEventUserData, so Shabang's own event tap ignores it.
...
extern const CGKeyCode SBKeyCodeTextCarrier;   // 0: carries a unicode string
extern const CGKeyCode SBKeyCodeANSIG;         // 5: with Command+Shift = "Go to Folder" in an open panel
extern const CGKeyCode SBKeyCodeReturn;        // 36
extern const CGKeyCode SBKeyCodeBackspace;     // 51
extern const CGKeyCode SBKeyCodeDownArrow;     // 125
extern const CGKeyCode SBKeyCodeUpArrow;       // 126
// Escape (53) is SBKeyCodeEscape in SBEventTap.h.

typedef NS_ENUM(NSInteger, SBKeyStrokeKind) {
    SBKeyStrokeKindText = 1,
    SBKeyStrokeKindEscape,
    SBKeyStrokeKindDownArrow,
    SBKeyStrokeKindUpArrow,
    SBKeyStrokeKindBackspace,
    SBKeyStrokeKindGoToFolder,   // Command+Shift+G
    SBKeyStrokeKindReturn,       // open panel go-to sheet / Open button, or an OPEN combobox list only
};

@interface SBKeyStroke : NSObject
...
@property (nonatomic, readonly) SBKeyStrokeKind kind;
...
extern NSString *const SBKeyBurstReasonEmpty;          // nothing to post
extern NSString *const SBKeyBurstReasonMalformed;      // a Return that is not alone, text with a control character
extern NSString *const SBKeyBurstReasonGuardRefused;   // the guard said no (focus, app, user key...)
extern NSString *const SBKeyBurstReasonPostFailed;     // the system would not take the event

@interface SBKeyBurstResult : NSObject
...
typedef BOOL (^SBKeyGuard)(SBKeyStroke *stroke, pid_t frontmostPID, id<SBAXNode> _Nullable focused);
...
typedef BOOL (^SBKeyLastCheck)(void);

@protocol SBKeyPosting <NSObject>
/// Posts `strokes` in order on the calling thread (the main thread in Shabang). Stops at the first refused guard or
...
- (SBKeyBurstResult *)postBurst:(NSArray<SBKeyStroke *> *)strokes guard:(SBKeyGuard)guard lastCheck:(nullable SBKeyLastCheck)lastCheck;
@end

/// Live desktop state. Every call reads the system again; nothing is cached.
@protocol SBDesktopState <NSObject>
...
- (nullable id<SBAXNode>)focusedElement;
...
@interface SBLiveDesktopState : NSObject <SBDesktopState>
@end

/// Settable state for tests.
@interface SBFakeDesktopState : NSObject <SBDesktopState>
@property (nonatomic) pid_t frontmostPID;
@property (nonatomic, strong, nullable) id<SBAXNode> focusedNode;
/// pid -> windows.
@property (nonatomic, readonly) NSMutableDictionary<NSNumber *, NSMutableArray<id<SBAXNode>> *> *windowsByPID;
...
@protocol SBKeyEventSink <NSObject>
- (BOOL)sendKeyCode:(CGKeyCode)keyCode flags:(CGEventFlags)flags text:(nullable NSString *)text;
@end

/// CGEventPost at the HID level from a private event source, every event tagged with SBSyntheticEventUserData.
@interface SBTaggedKeyEventSink : NSObject <SBKeyEventSink>
@end

@interface SBKeyPoster : NSObject <SBKeyPosting>
- (instancetype)init NS_UNAVAILABLE;
- (instancetype)initWithState:(id<SBDesktopState>)state sink:(id<SBKeyEventSink>)sink NS_DESIGNATED_INITIALIZER;
/// SBLiveDesktopState + SBTaggedKeyEventSink, 2 ms between posts (some web views drop bursts).
+ (SBKeyPoster *)livePoster;
@property (nonatomic, readonly) id<SBDesktopState> state;
@property (nonatomic, readonly) id<SBKeyEventSink> sink;
...
+ (nullable NSArray<SBKeyStroke *> *)atomicStrokesForBurst:(NSArray<SBKeyStroke *> *)strokes reason:(NSString *_Nullable *_Nullable)reason;
@end

/// The production poster over a SBFakeDesktopState and a recording sink: the guard logic under test is the real one.
@interface SBFakeKeyPoster : SBKeyPoster
- (instancetype)initWithState:(SBFakeDesktopState *)state NS_DESIGNATED_INITIALIZER;
- (instancetype)initWithState:(id<SBDesktopState>)state sink:(id<SBKeyEventSink>)sink NS_UNAVAILABLE;
@property (nonatomic, readonly) SBFakeDesktopState *fakeState;
/// Every atomic post that reached the "system", in order (text chunks as text strokes).
@property (nonatomic, readonly, copy) NSArray<SBKeyStroke *> *posted;
