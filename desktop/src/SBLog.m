...
NSString *const SBProductName = @"Shabang";
...
static dispatch_queue_t SBLogQueue(void) {
...
static NSString *SBDefaultLogPath(void) {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Logs/Shabang/desktop.log"];
}

NSString *SBLogPath(void) {
    __block NSString *path;
    dispatch_sync(SBLogQueue(), ^{ path = gPath ?: SBDefaultLogPath(); });
    return path;
}

void SBLogSetPath(NSString *path) {
    dispatch_sync(SBLogQueue(), ^{ gPath = [path copy]; });
}

void SBLogSetMirrorToStderr(BOOL mirror) {
    dispatch_sync(SBLogQueue(), ^{ gMirror = mirror; gMirrorSet = YES; });
}

void SBLogSetMaxBytes(unsigned long long maxBytes) {
    dispatch_sync(SBLogQueue(), ^{ gMaxBytes = maxBytes; });
}

static BOOL SBShouldMirror(void) {
    if (gMirrorSet) return gMirror;
    const char *env = getenv("SHABANG_LOG_STDERR");
    return env && env[0] == '1';
}

static void SBRotateIfNeeded(NSString *path) {
    struct stat st;
...
static void SBAppendLine(NSString *line) {
    NSString *path = gPath ?: SBDefaultLogPath();
    NSFileManager *fm = NSFileManager.defaultManager;
...
void SBLog(NSString *format, ...) {
    ...
    dispatch_async(SBLogQueue(), ^{
        NSString *oneLine = [message stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
        NSString *line = [NSString stringWithFormat:@"%@ %@\n", [formatter stringFromDate:now], oneLine];
        SBAppendLine(line);
        if (SBShouldMirror()) fputs(line.UTF8String, stderr);
    });
}

void SBLogFlush(void) {
    dispatch_sync(SBLogQueue(), ^{});
}

NSString *SBLogLabel(NSString *label) {
