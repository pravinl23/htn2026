// SBNextAction: Shabang anywhere on the native side (docs/anywhere.md). When the form walk has nothing to offer
// -- and most windows are not forms -- this is what Shabang proposes instead: the one control the affordances,
// the kind of place and the user's own role memory say comes next.
//
//   capture (SBCapture)  ->  hints (SBAffordance)  ->  GhostCore.nextAction  ->  one click ghost
//
...
/// Irreversible: drawn with the lock badge, never pressed by Shabang.
...
/// A `click` SBGhost for the walk: unlocked ones are pressed, locked ones only ever parked on.
...
/// a reason to stop proposing. Written atomically with mode 0600, like every other Shabang file.
@interface SBRoleMemoryStore : NSObject
- (instancetype)init NS_UNAVAILABLE;
- (instancetype)initWithPath:(NSString *)path core:(nullable SBCore *)core NS_DESIGNATED_INITIALIZER;
...
extern NSString *const SBRoleOutcomeAccepted;
extern NSString *const SBRoleOutcomeDismissed;
extern NSString *const SBRoleOutcomeReplaced;

@interface SBNextAction : NSObject

- (instancetype)init NS_UNAVAILABLE;
/// `memory` may be nil: then nothing is remembered and every proposal comes from the priors alone.
- (instancetype)initWithCore:(SBCore *)core memory:(nullable SBRoleMemoryStore *)memory NS_DESIGNATED_INITIALIZER;
...
- (nullable SBNextProposal *)proposeForResult:(nullable SBCaptureResult *)result
                                       window:(nullable id<SBAXNode>)window
                                      signals:(nullable SBPageSignals *)signals;
...
@property (nonatomic, readonly, copy) NSArray<SBNextProposal *> *ranked;
/// What the last pass measured, for the HUD and the tests.
@property (nonatomic, readonly, copy, nullable) SBPageSignals *lastSignals;
...
- (void)recordOutcome:(NSString *)outcome forProposal:(nullable SBNextProposal *)proposal;

// ---------- pure, exposed for tests ----------
/// An empty box somebody types in that the app has put the keyboard into. `outSignature` gets its signature.
+ (BOOL)window:(SBCaptureResult *)result hasAFocusedEmptyField:(NSString *_Nullable *_Nullable)outSignature;
