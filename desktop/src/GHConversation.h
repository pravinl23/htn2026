// GHConversation: reading a thread of messages off the screen, so the reply can be drafted.
//
// The sibling of GHPageContext. That one reads what a job posting is about; this one reads what the last few
// messages said, so `/v1/ghost-text` can draft an answer to them.
//
...
// bubble inside it. An app that names its messages some other way simply yields no conversation, and Ghost
...
extern const NSUInteger GHConversationMaxMessages;      // 14
/// Per message, and for the whole thread. A wall of text makes a worse draft, not a better one.
extern const NSUInteger GHConversationMaxMessageChars;  // 400
extern const NSUInteger GHConversationMaxTotalChars;    // 2000
extern const NSUInteger GHConversationMaxNodes;         // 4000
extern const NSTimeInterval GHConversationMaxSeconds;   // 0.20

@interface GHMessage : NSObject
...
+ (nullable GHMessage *)messageFromDescription:(nullable NSString *)description;
