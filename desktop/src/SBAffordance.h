// SBAffordance: what the window in front OFFERS, read off the accessibility tree (docs/anywhere.md sections 2
// and 3). Capture already turns a window into SBFields; this adds the generic hints the shared affordance layer
...
@interface SBPageSignals : NSObject <NSCopying>
...
///
/// This is the sequence signal Shabang can always read, and it needs no history at all: an app that opens a
...
@interface SBAffordance : NSObject
...
+ (SBPageSignals *)annotateResult:(nullable SBCaptureResult *)result window:(nullable id<SBAXNode>)window;
+ (SBPageSignals *)annotateResult:(nullable SBCaptureResult *)result
