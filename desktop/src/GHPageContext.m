...
const NSUInteger GHPageContextMaxDescription = 2000;
const NSUInteger GHPageContextMaxNodes = 6000;
const NSTimeInterval GHPageContextMaxSeconds = 0.25;
...
typedef NS_OPTIONS(NSUInteger, GHContextFlags) {
    GHContextInLink = 1 << 0,     // link or button text: navigation, not description
    GHContextInFooter = 1 << 1,   // AXLandmarkContentInfo: "Powered by <job board>"
    GHContextInRoleHeading = 1 << 2,
};

@interface GHPageContext ()
...
static BOOL GHControlRole(NSString *role) {
...
static NSString *GHFirstMatch(NSString *text, NSString *pattern) {
...
static NSString *GHName(NSString *text) {
    NSString *name = [GHPageContext normalizedText:text];
    name = [name stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@" .,;:-|"]];
    if (name.length == 0) return nil;
    return name.length > kMaxNameLength ? [GHPageContext text:name cappedAt:kMaxNameLength] : name;
}

@implementation GHPageContext
...
+ (NSString *)companyFromTitle:(NSString *)title role:(NSString *)role {
    ...
    NSString *rest = GHFirstMatch(text, @"^job application for (.+)$");
    if (!rest) return nil;
...
static id<GHAXNode> GHFindWebArea(id<GHAXNode> root, GHAXWalkBudget *outer) {
    if ([root.role isEqualToString:@"AXWebArea"]) return root;
    GHAXWalkBudget budget = GHAXWalkBudgetNested(outer, kWebAreaSearchNodes);
...
static NSString *GHHeadingText(id<GHAXNode> heading) {
    NSString *title = [GHPageContext normalizedText:heading.title];
    if (title.length) return title;
    NSMutableArray<NSString *> *parts = [NSMutableArray array];
    for (id<GHAXNode> child in heading.children) {
        NSString *text = [GHPageContext normalizedText:[child.role isEqualToString:@"AXStaticText"] ? child.value : child.title];
        if (text.length) [parts addObject:text];
    }
    return [GHPageContext normalizedText:[parts componentsJoinedByString:@" "]];
}

static BOOL GHIsApplyHeading(NSString *text) {
    return GHFirstMatch(text, @"^(apply (for|to) this (job|position|role|opening))\\W*$") != nil;
}

+ (instancetype)contextFromNode:(id<GHAXNode>)root {
    return [self contextFromNode:root maxNodes:GHPageContextMaxNodes];
}

+ (instancetype)contextFromNode:(id<GHAXNode>)root maxNodes:(NSUInteger)maxNodes {
    GHPageContext *context = [[self alloc] initPrivately];
    // Nodes AND wall clock (it runs on the main thread): a huge page or a hung app ends the walk early.
    GHAXWalkBudget budget = GHAXWalkBudgetMake(maxNodes, GHPageContextMaxSeconds);
    id<GHAXNode> page = GHFindWebArea(root, &budget) ?: root;
    ...
    NSMutableArray<id<GHAXNode>> *stack = [NSMutableArray arrayWithObject:page];
    ...
    while (stack.count) {
        ...
        GHContextFlags flag = flags.lastObject.unsignedIntegerValue;
        [stack removeLastObject];
        [depths removeLastObject];
        [flags removeLastObject];
        if (!GHAXWalkBudgetSpend(&budget, node)) { context.truncated = YES; break; }
        visited++;

        NSString *nodeRole = node.role ?: @"";
        if ([node.subrole isEqualToString:@"AXLandmarkContentInfo"]) flag |= GHContextInFooter;
        if ([nodeRole isEqualToString:@"AXLink"] || [nodeRole isEqualToString:@"AXButton"]) flag |= GHContextInLink;

        if ([nodeRole isEqualToString:@"AXHeading"]) {
            NSString *text = GHHeadingText(node);
            if (GHIsApplyHeading(text)) { applyHeadingSeen = YES; break; }   // everything after it is the form
            if (!role && text.length) { role = GHName(text); flag |= GHContextInRoleHeading; }
        }
        if (!logoCompany && !(flag & GHContextInFooter) && ([nodeRole isEqualToString:@"AXLink"] || [nodeRole isEqualToString:@"AXImage"])) {
            logoCompany = [self companyFromLogoText:node.title] ?: [self companyFromLogoText:node.axDescription];
        }
        if (formStart < 0 && (GHControlRole(nodeRole) || [node.subrole isEqualToString:@"AXLandmarkForm"])) formStart = (NSInteger)pieces.count;
        if ([nodeRole isEqualToString:@"AXStaticText"]) {
            ...
            if (text.length && !(flag & (GHContextInLink | GHContextInFooter | GHContextInRoleHeading))) [pieces addObject:text];
            continue;
        }
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<GHPageContext company=%lu role=%lu description=%lu>", (unsigned long)self.company.length,
