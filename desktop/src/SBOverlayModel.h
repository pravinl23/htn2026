// SBOverlayModel: pure view-model of the overlay. Turns (ghosts + AX rects + current index + HUD info) into a flat
...
typedef NS_ENUM(NSInteger, SBOverlayMode) {
    SBOverlayModeText,        // gray text inside a single-line field
    SBOverlayModeMultiline,   // wrapped gray text inside a text area
    SBOverlayModeSelectPill,  // pill inside a popup / combo box, left of its arrow
    SBOverlayModePill,        // pill beside a checkbox or radio group, or an upload control (the file name)
    SBOverlayModeTarget,      // button or link: ring and cursor only (the parked lock ghost)
};

SBOverlayMode SBOverlayModeForKind(NSString *_Nullable kind);
...
@interface SBOverlayEntry : NSObject
...
@property (nonatomic) BOOL streaming;               // Shabang.pending: free text still streaming in
...
@interface SBOverlayInput : NSObject
@property (nonatomic, copy) NSArray<SBOverlayEntry *> *entries;   // live ghosts in walk order, lock ghost last
...
@property (nonatomic, strong, nullable) SBOverlayHUDInfo *hud;
...
typedef NS_ENUM(NSInteger, SBDrawKind) {
    SBDrawKindGhostText,
    SBDrawKindPill,
    SBDrawKindRing,
    SBDrawKindCursor,
...
typedef NS_ENUM(NSInteger, SBDrawAnchor) {
    SBDrawAnchorFill,
    SBDrawAnchorLeftCenter,
    SBDrawAnchorRightCenter,
    SBDrawAnchorTopLeft,
    SBDrawAnchorBottomRight,
};
...
@property (nonatomic, readonly) SBDrawAnchor anchor;
...
@interface SBOverlayModel : NSObject
+ (instancetype)modelWithInput:(SBOverlayInput *)input layout:(SBScreenLayout *)layout;
+ (instancetype)emptyModel;
@property (nonatomic, copy, readonly) NSArray<SBDrawItem *> *items;
@property (nonatomic, copy, readonly) NSString *layoutFingerprint;
/// True when the current ghost is visible enough to draw. SBEventTap may only consume Tab while this holds.
...
- (nullable SBDrawItem *)itemWithKey:(NSString *)key screen:(NSUInteger)screenIndex;
@end

/// Keyed diff of two item lists (of ONE panel). Rendering the same model twice yields an empty diff.
@interface SBOverlayDiff : NSObject
+ (instancetype)diffFromItems:(NSArray<SBDrawItem *> *)oldItems toItems:(NSArray<SBDrawItem *> *)newItems;
@property (nonatomic, copy, readonly) NSArray<SBDrawItem *> *added;
@property (nonatomic, copy, readonly) NSArray<SBDrawItem *> *changed;     // same key, new content: reuse the layer
@property (nonatomic, copy, readonly) NSArray<SBDrawItem *> *unchanged;
