...
static NSDictionary<NSString *, id> *GHColdStartHistoryRow(NSString *url, long long webkitMicroseconds, NSString *fromURL);
/// "/watch/1234/abcdef" -> "/watch/:id/:id": the shape a page kind can be guessed from, never the page.
static NSString *GHColdStartPathPattern(NSString *path);
/// The `url = ` lines of a git config. Nothing else in the file is looked at.
static NSArray<NSString *> *GHColdStartRemotesInGitConfig(NSString *text);
/// Letters only, so a scan id can never be mistaken for a number, a path or anything personal.
static NSString *GHColdStartNewScanIdentifier(void);
static NSDate *_Nullable GHColdStartParseMetadataDate(id value);

NSString *GHColdStartStopName(GHColdStartStop stop) {
    switch (stop) {
        case GHColdStartStopFinished: return @"finished";
        case GHColdStartStopBudget: return @"budget";
        case GHColdStartStopCancelled: return @"cancelled";
        case GHColdStartStopNoCore: return @"no-core";
    }
    return @"finished";
}

GHColdStartBudget GHColdStartDefaultBudget(void) {
    return (GHColdStartBudget){ .wallClockSeconds = 60.0, .maxFiles = 500, .maxBytesPerFile = 2 * 1024 * 1024 };
}

#pragma mark - the real files

/// Extensions read straight off disk. Small, plain, and no subprocess.
static NSSet<NSString *> *GHPlainTextExtensions(void) {
...
static NSSet<NSString *> *GHTextutilExtensions(void) {
    static NSSet<NSString *> *set;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
...
@implementation GHColdStartFilesMac
...
- (nullable NSString *)runTool:(NSString *)tool arguments:(NSArray<NSString *> *)arguments maxBytes:(NSUInteger)maxBytes {
    ...
    if (dispatch_semaphore_wait(done, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.subprocessTimeout * NSEC_PER_SEC))) != 0) {
        [task terminate];
        GHLog(@"cold-start: %@ timed out", tool.lastPathComponent);
        return nil;
    }
    [task waitUntilExit];
...
- (nullable NSString *)textOfFileAtPath:(NSString *)path maxBytes:(NSUInteger)maxBytes {
    ...
    if ([GHPlainTextExtensions() containsObject:extension]) {
    ...
    if ([GHTextutilExtensions() containsObject:extension]) {
...
- (NSArray<NSDictionary<NSString *, id> *> *)historyRowsFromCopyAtPath:(NSString *)copyPath limit:(NSUInteger)limit {
    ...
    for (NSString *line in [text componentsSeparatedByString:newline]) {
        ...
        NSDictionary *row = GHColdStartHistoryRow(columns[0], [columns[1] longLongValue], columns.count > 2 ? columns[2] : @"");
...
static NSDictionary<NSString *, id> *GHColdStartHistoryRow(NSString *url, long long webkitMicroseconds, NSString *fromURL) {
    ...
    NSString *pattern = GHColdStartPathPattern(components.path);
...
@implementation GHColdStartProposal
...
@implementation GHColdStartResult
...
- (NSDictionary<NSString *, id> *)reportObject {
    ...
    for (GHColdStartProposal *proposal in self.proposals) [proposals addObject:proposal.reportObject];
    ...
    report[@"budget"] = @{
        ...
        @"stop": GHColdStartStopName(self.stop),
    };
...
- (NSDictionary<NSString *, id> *)pendingObject {
    NSMutableArray *proposals = [NSMutableArray array];
    for (GHColdStartProposal *proposal in self.proposals) [proposals addObject:proposal.pendingObject];
...
static NSString *_Nullable GHColdStartLeakInString(NSString *text) {
...
BOOL GHColdStartIsValueFree(id object, NSString **offender) {
    if ([object isKindOfClass:NSString.class]) {
        NSString *leak = GHColdStartLeakInString(object);
        if (leak) {
    ...
    if ([object isKindOfClass:NSArray.class]) {
        for (id item in (NSArray *)object) if (!GHColdStartIsValueFree(item, offender)) return NO;
        return YES;
    }
    if ([object isKindOfClass:NSDictionary.class]) {
        ...
        [(NSDictionary *)object enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL *stop) {
            NSString *leak = nil;
            if (!GHColdStartIsValueFree(key, &leak) || !GHColdStartIsValueFree(value, &leak)) {
                clean = NO;
                found = leak;
                *stop = YES;
            }
...
@interface GHColdStartSystemClock : NSObject <GHColdStartClock>
@end

@implementation GHColdStartSystemClock
- (NSTimeInterval)nowSeconds {
    return (NSTimeInterval)clock_gettime_nsec_np(CLOCK_MONOTONIC) / NSEC_PER_SEC;
}
@end

@interface GHColdStart ()
@property (nonatomic, strong, nullable) GHCore *core;
@property (nonatomic, strong) id<GHColdStartFiles> files;
@property (nonatomic) NSTimeInterval startedAt;
@property (nonatomic) NSUInteger filesOpened;
@end

@implementation GHColdStart {
    _Atomic(BOOL) _cancelled;
}

- (instancetype)initWithCore:(GHCore *)core files:(id<GHColdStartFiles>)files {
    if ((self = [super init])) {
        _core = core;
        _files = files;
        _budget = GHColdStartDefaultBudget();
        _clock = [[GHColdStartSystemClock alloc] init];
...
- (BOOL)mayContinue:(GHColdStartStop *)stop {
    if (self.cancelled) {
        if (stop) *stop = GHColdStartStopCancelled;
        return NO;
    }
    if ([self elapsed] >= self.budget.wallClockSeconds) {
        if (stop) *stop = GHColdStartStopBudget;
        return NO;
    }
...
- (nullable NSDictionary<NSString *, id> *)planForSources:(NSArray<GHScanSource *> *)sources enabledKinds:(NSSet<NSString *> *)enabledKinds {
    NSMutableArray *descriptors = [NSMutableArray array];
    for (GHScanSource *source in sources) {
...
- (NSString *)originJSONForKind:(NSString *)kind fileName:(NSString *)fileName {
    return GHJSONString(@{ @"sourceKind": kind, @"fileName": fileName ?: @"local file" }) ?: @"{}";
}
...
- (nullable NSDictionary *)extractionFrom:(NSString *)function arguments:(NSArray<NSString *> *)arguments {
    ...
    id result = GHJSONParse(answer);
    return [result isKindOfClass:NSDictionary.class] ? result : nil;
}

- (GHColdStartResult *)runSources:(NSArray<GHScanSource *> *)sources enabledKinds:(NSSet<NSString *> *)enabledKinds {
    GHColdStartResult *result = [[GHColdStartResult alloc] init];
    result.scanIdentifier = GHColdStartNewScanIdentifier();
    ...
    result.plan = plan;
    if (!self.core || !plan) {
        result.stop = GHColdStartStopNoCore;
        result.elapsedSeconds = [self elapsed];
        GHLog(@"cold-start: no core bundle; nothing was read");
        return result;
    }

    NSMutableDictionary<NSString *, GHScanSource *> *byKind = [NSMutableDictionary dictionary];
    for (GHScanSource *source in sources) byKind[source.kind] = source;
    ...
    GHColdStartStop stop = GHColdStartStopFinished;

    for (NSDictionary *row in (NSArray *)(plan[@"sources"] ?: @[])) {
        NSString *kind = row[@"kind"];
        GHScanSource *source = byKind[kind];
        ...
        } else if ([kind isEqualToString:GHScanKindDock]) {
            [observations addObjectsFromArray:[self readDock:source opened:&opened]];
        } else if ([kind isEqualToString:GHScanKindLoginItems]) {
            [observations addObjectsFromArray:[self readLoginItems:source limit:planned opened:&opened]];
        } else if ([kind isEqualToString:GHScanKindRecentApps]) {
            [observations addObjectsFromArray:[self readRecentApps:source limit:planned]];
        } else if ([kind isEqualToString:GHScanKindRecentDocs]) {
            [screenKinds addObjectsFromArray:[self readRecentDocumentKinds]];
        } else if ([kind isEqualToString:GHScanKindAppInventory]) {
            [observations addObjectsFromArray:[self readAppInventory:source limit:planned opened:&opened stop:&stop]];
        }
        if ([kind isEqualToString:GHScanKindDock] || [kind isEqualToString:GHScanKindLoginItems] ||
            [kind isEqualToString:GHScanKindRecentApps] || [kind isEqualToString:GHScanKindAppInventory]) {
        ...
        [reports addObject:report];
        if (stop != GHColdStartStopFinished) break;
    }
    ...
    if (observations.count > 0) {
        ...
        NSString *answer = [self.core callString:@"coldStartSurfaces"
                                       arguments:@[ GHJSONString(observations) ?: @"[]", GHJSONString(transitions) ?: @"[]", @"{}" ]];
        id aggregate = GHJSONParse(answer);
    ...
    NSString *mergedJSON = [self.core callString:@"coldStartMerge" arguments:@[ GHJSONString(extractions) ?: @"[]" ]];
    NSDictionary *merged = GHJSONParse(mergedJSON);
    NSArray *proposals = [merged isKindOfClass:NSDictionary.class] ? merged[@"proposals"] : @[];
    NSMutableArray<GHColdStartProposal *> *out = [NSMutableArray array];
    NSUInteger index = 0;
    for (NSDictionary *raw in (proposals ?: @[])) {
        if (![raw isKindOfClass:NSDictionary.class]) continue;
        GHColdStartProposal *proposal = [[GHColdStartProposal alloc] init];
    ...
    result.elapsedSeconds = [self elapsed];
    result.stop = stop;
    GHLog(@"cold-start: %lu proposals from %lu files in %.1f s (%@), %ld skipped as sensitive",
          (unsigned long)out.count, (unsigned long)self.filesOpened, result.elapsedSeconds, GHColdStartStopName(stop), (long)skippedTotal);
    return result;
}

- (void)runSources:(NSArray<GHScanSource *> *)sources
      enabledKinds:(NSSet<NSString *> *)enabledKinds
        completion:(void (^)(GHColdStartResult *))completion {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        GHColdStartResult *result = [self runSources:sources enabledKinds:enabledKinds];
...
- (NSArray<NSDictionary *> *)readContacts:(GHScanSource *)source
    ...
    NSDictionary *extraction = [self extractionFrom:@"coldStartVCard"
                                          arguments:@[ text, [self originJSONForKind:GHScanKindContacts fileName:fileName], @"{\"firstCardOnly\":true}" ]];
    return extraction ? @[ extraction ] : @[];
}

/// Tier 2. Résumé-shaped documents: the name is screened, then the text, then the extractor.
- (NSArray<NSDictionary *> *)readDocuments:(GHScanSource *)source
    ...
    for (NSString *path in source.paths) {
        ...
        NSDictionary *extraction = [self extractionFrom:@"coldStartResumeText"
                                              arguments:@[ text, [self originJSONForKind:GHScanKindResume fileName:fileName],
                                                           GHJSONString(@{ @"fileName": fileName }) ?: @"{}" ]];
...
- (NSArray<NSDictionary *> *)readProjects:(GHScanSource *)source
                                    limit:(NSUInteger)limit
                                 maxBytes:(NSUInteger)maxBytes
                                   opened:(NSUInteger *)opened
                                  skipped:(NSMutableDictionary<NSString *, NSNumber *> *)skipped
    ...
    for (NSString *path in source.paths) {
        ...
        if ([manifest isKindOfClass:NSDictionary.class] && manifest[@"author"]) {
            ...
            NSString *authorJSON = GHJSONString(@{ @"author": manifest[@"author"] });
            if (authorJSON) {
                NSDictionary *extraction = [self extractionFrom:@"coldStartPackageAuthor"
                                                      arguments:@[ authorJSON, [self originJSONForKind:GHScanKindProjects fileName:@"package.json"] ]];
                if (extraction) [out addObject:extraction];
            }
        }
        ...
        self.filesOpened++;
        for (NSString *remote in GHColdStartRemotesInGitConfig(configText)) {
            ...
            NSDictionary *extraction = [self extractionFrom:@"coldStartGitRemote"
                                                  arguments:@[ remote, [self originJSONForKind:GHScanKindProjects fileName:@"git config"],
                                                               @"{\"ownerKind\":\"unknown\"}" ]];
            if (extraction) [out addObject:extraction];
        }
    }
...
static NSDictionary *GHSurfaceObservation(NSString *bundleIdentifier, NSString *source, NSInteger visits) {
...
- (NSArray<NSDictionary *> *)readDock:(GHScanSource *)source opened:(NSUInteger *)opened {
    id<GHScanEnvironment> environment = self.environment;
    NSString *path = source.paths.firstObject;
    ...
    for (NSUInteger i = 0; i < sections.count; i++) {
        ...
        for (id entry in (NSArray *)entries) {
            ...
            NSDictionary *observation = GHSurfaceObservation(identifier, GHScanKindDock, weights[i].integerValue);
...
- (NSArray<NSDictionary *> *)readLoginItems:(GHScanSource *)source limit:(NSUInteger)limit opened:(NSUInteger *)opened {
    id<GHScanEnvironment> environment = self.environment;
    ...
    for (NSString *name in [environment entryNamesAtDirectoryPath:directory]) {
        ...
        NSDictionary *observation = GHSurfaceObservation(name.stringByDeletingPathExtension, GHScanKindLoginItems, kLoginItemVisits);
        if (observation) [out addObject:observation];
    }
    (*opened) += out.count;
    return out;
...
- (NSArray<NSDictionary *> *)readRecentApps:(GHScanSource *)source limit:(NSUInteger)limit {
    id<GHScanEnvironment> environment = self.environment;
    if (!environment || limit == 0) return @[];
    NSArray<NSString *> *paths = [environment pathsForSystemSpotlightQuery:[GHScanSources queryForRecentApplications]
    ...
    for (NSDictionary *record in [environment metadataForPaths:paths attributes:attributes]) {
        ...
        NSMutableDictionary *observation = [GHSurfaceObservation(identifier, GHScanKindRecentApps, uses) mutableCopy];
        if (!observation) continue;
        NSDate *last = GHColdStartParseMetadataDate(record[@"kMDItemLastUsedDate"]);
        if (last) {
...
- (NSArray<NSDictionary *> *)readRecentDocumentKinds {
    ...
    NSMutableArray<NSDictionary *> *out = [NSMutableArray array];
    NSDictionary<NSString *, NSString *> *queries = [GHScanSources queriesForRecentDocumentClasses];
...
/// person owns but has never been seen using becomes a surface Ghost can recognise, and never a habit.
- (NSArray<NSDictionary *> *)readAppInventory:(GHScanSource *)source
                                        limit:(NSUInteger)limit
                                       opened:(NSUInteger *)opened
                                         stop:(GHColdStartStop *)stop {
    id<GHScanEnvironment> environment = self.environment;
    ...
    for (NSString *root in source.paths) {
        for (NSString *name in [environment entryNamesAtDirectoryPath:root]) {
            ...
            if (![identifier isKindOfClass:NSString.class]) continue;
            NSMutableDictionary *observation = [GHSurfaceObservation(identifier, GHScanKindAppInventory, 0) mutableCopy];
            if (!observation) continue;
...
- (nullable NSDictionary *)readHistory:(GHScanSource *)source
                                 limit:(NSUInteger)limit
    ...
    NSString *options = GHJSONString(@{ @"timeZoneOffsetMinutes": @(self.timeZoneOffsetMinutes), @"minVisits": @(self.habitMinVisits) }) ?: @"{}";
    NSString *answer = [self.core callString:@"coldStartHabits" arguments:@[ GHJSONString(rows) ?: @"[]", options ]];
    id aggregate = GHJSONParse(answer);
...
GHColdStartApplyCounts GHColdStartApply(NSDictionary *report, NSDictionary *pending, GHProfileStore *store, NSError **error) {
    GHColdStartApplyCounts counts = (GHColdStartApplyCounts){ 0, 0, 0, 0 };
    ...
    if (![reportScan isKindOfClass:NSString.class] || ![pendingScan isKindOfClass:NSString.class] || ![reportScan isEqualToString:pendingScan]) {
        if (error) {
            *error = [NSError errorWithDomain:@"GHColdStart" code:1
...
BOOL GHColdStartSeedRoleMemory(NSDictionary *pending, NSString *memoryPath) {
    ...
    NSString *json = GHJSONString(memory);
    NSData *data = [json dataUsingEncoding:NSUTF8StringEncoding];
    return data ? GHWritePrivateFile(memoryPath, data, NULL) : NO;
}
...
static NSString *GHReadFileText(NSString *path) {
    NSString *text = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:NULL];
    return text ?: @"";
}

static NSDictionary *_Nullable GHCallGraph(GHCore *core, NSString *function, NSArray<NSString *> *arguments) {
    if (!core) return nil;
    id answer = GHJSONParse([core callString:function arguments:arguments]);
...
BOOL GHColdStartApplyGraph(GHCore *core, NSDictionary *report, NSDictionary *pending, NSString *graphPath,
    ...
    // have to agree before a single fact moves, exactly as GHColdStartApply requires for the profile.
    ...
    input[@"file"] = GHReadFileText(graphPath);
    ...
    NSString *json = GHJSONString(input);
    NSDictionary *result = json ? GHCallGraph(core, @"coldStartGraphApply", @[ json ]) : nil;
    NSString *text = [result[@"file"] isKindOfClass:NSString.class] ? result[@"file"] : nil;
    if (text.length == 0) return NO;
    NSData *data = [text dataUsingEncoding:NSUTF8StringEncoding];
    if (!data || !GHWritePrivateFile(graphPath, data, NULL)) return NO;
    if (summary) {
    ...
    GHLog(@"cold-start: graph now holds %@ surfaces and %@ habits (%@ bytes)",
...
NSDictionary<NSString *, id> *_Nullable GHColdStartDescribeGraph(GHCore *core, NSString *graphPath) {
    return GHCallGraph(core, @"coldStartGraphDescribe", @[ GHReadFileText(graphPath), @"10" ]);
}

BOOL GHColdStartForgetSource(GHCore *core, NSString *graphPath, NSString *kind, NSDictionary<NSString *, id> **removed) {
    if (!core || graphPath.length == 0 || kind.length == 0) return NO;
    NSDictionary *result = GHCallGraph(core, @"coldStartGraphForget", @[ GHReadFileText(graphPath), kind ]);
    NSString *text = [result[@"file"] isKindOfClass:NSString.class] ? result[@"file"] : nil;
    if (text.length == 0) return NO;
    NSData *data = [text dataUsingEncoding:NSUTF8StringEncoding];
    if (!data || !GHWritePrivateFile(graphPath, data, NULL)) return NO;
...
BOOL GHColdStartForgetEverything(NSString *directory) {
    ...
    GHLog(@"cold-start: forget everything (%@)", removedSomething ? @"removed" : @"nothing to remove");
