...
static void SBProbeSettle(NSTimeInterval seconds) {
    [NSThread sleepForTimeInterval:seconds];
}

static void SBProbeCollectComboBoxes(id<SBAXNode> node, NSMutableArray<id<SBAXNode>> *into, SBAXWalkBudget *budget, NSUInteger limit) {
    if (into.count >= limit || !SBAXWalkBudgetSpend(budget, node)) return;
    if ([SBComboBoxDriver isComboBox:node]) [into addObject:node];
    for (id<SBAXNode> child in node.children) {
        if (into.count >= limit) return;
        SBProbeCollectComboBoxes(child, into, budget, limit);
    }
}

@implementation SBProbe

+ (NSArray<id<SBAXNode>> *)comboBoxesUnderWindow:(id<SBAXNode>)window limit:(NSUInteger)limit {
    NSMutableArray<id<SBAXNode>> *found = [NSMutableArray array];
    SBAXWalkBudget budget = SBAXWalkBudgetMake(kProbeWalkNodes, kProbeWalkSeconds);
    if (window) SBProbeCollectComboBoxes(window, found, &budget, limit);
    return found;
}

+ (NSString *)labelOfComboBox:(id<SBAXNode>)node {
...
+ (id<SBAXNode>)toggleButtonForComboBox:(id<SBAXNode>)comboBox {
    ...
    for (NSUInteger i = index + 1; i < siblings.count && i <= index + kProbeSiblingWindow; i++) {
        ...
        if ([SBComboBoxDriver isComboBox:sibling] || [sibling.role isEqualToString:(__bridge NSString *)kAXTextFieldRole]) return nil;
        if ([sibling.role isEqualToString:(__bridge NSString *)kAXButtonRole]) return sibling;
    }
    return nil;
}

+ (NSDictionary<NSString *, id> *)summaryOfNode:(id<SBAXNode>)node {
...
+ (NSArray<NSDictionary *> *)neighbourhoodOf:(id<SBAXNode>)comboBox {
    id<SBAXNode> parent = comboBox.parent;
    NSArray<id<SBAXNode>> *siblings = parent.children;
    NSUInteger index = NSNotFound;
    for (NSUInteger i = 0; i < siblings.count; i++) if ([siblings[i] isSameNode:comboBox]) { index = i; break; }
...
+ (NSDictionary<NSString *, id> *)shallowTreeOf:(id<SBAXNode>)node depth:(NSUInteger)depth {
...
+ (NSArray<NSDictionary *> *)listCandidatesAfter:(id<SBAXNode>)comboBox {
    id<SBAXNode> parent = comboBox.parent;
    NSArray<id<SBAXNode>> *siblings = parent.children;
    NSUInteger index = NSNotFound;
    for (NSUInteger i = 0; i < siblings.count; i++) if ([siblings[i] isSameNode:comboBox]) { index = i; break; }
    ...
    for (NSUInteger i = index + 1; i < siblings.count && i <= index + 8; i++) {
        id<SBAXNode> sibling = siblings[i];
        NSArray<id<SBAXNode>> *options = [SBComboBoxDriver optionsInList:sibling];
        NSMutableArray<NSString *> *texts = [NSMutableArray array];
        for (id<SBAXNode> option in options) {
            if (texts.count >= kProbeMaxOptionsReported) break;
            [texts addObject:[SBComboBoxDriver textOfOption:option] ?: @""];
        }
        NSMutableDictionary *entry = [@{ @"siblingIndex": @(i), @"role": sibling.role ?: @"",
                                         ...
                                         @"saysNothingFound": @([SBComboBoxDriver listSaysNothingFound:sibling]) } mutableCopy];
...
/// What SBComboBoxDriver itself would find right now: the answer to "can the shipping code see this list?".
+ (NSDictionary<NSString *, id> *)driverViewOf:(id<SBAXNode>)comboBox {
    id<SBAXNode> list = [SBComboBoxDriver listForComboBox:comboBox];
    NSMutableDictionary<NSString *, id> *out = [@{ @"listFound": @(list != nil) } mutableCopy];
    if (!list) return out;
    out[@"list"] = [self summaryOfNode:list];
    out[@"saysNothingFound"] = @([SBComboBoxDriver listSaysNothingFound:list]);
...
+ (NSDictionary<NSString *, id> *)systemFocus {
    ...
    NSDictionary *summary = [self summaryOfNode:[SBAXElementNode nodeWithElement:(AXUIElementRef)focused]];
    CFRelease(focused);
    return summary;
}

+ (NSDictionary<NSString *, id> *)probeComboBoxUnderWindow:(id<SBAXNode>)window labelSubstring:(NSString *)labelSubstring {
    NSMutableDictionary<NSString *, id> *report = [@{ @"want": labelSubstring ?: @"" } mutableCopy];
    NSArray<id<SBAXNode>> *comboBoxes = [self comboBoxesUnderWindow:window limit:40];
    NSMutableArray<NSString *> *seen = [NSMutableArray array];
    id<SBAXNode> target = nil;
    NSString *targetLabel = nil;
    for (id<SBAXNode> candidate in comboBoxes) {
    ...
    if ([SBComboBoxDriver isDemographicComboBox:target]) { report[@"error"] = @"demographic"; return report; }
    ...
    report[@"shownBefore"] = [SBComboBoxDriver shownTextsForComboBox:target typed:nil] ?: @[];
    ...
    report[@"focus"] = @{ @"setAccepted": @(focusSet),
                          @"nodeFocusedAfter": @([SBAXElementNode nodeWithElement:target.axElement].isFocused),
    ...
    SBProbeSettle(kProbeSettle);
    id<SBAXNode> afterPress = [SBAXElementNode nodeWithElement:target.axElement];
    ...
    id<SBAXNode> toggle = [self toggleButtonForComboBox:afterPress];
    ...
    if (!open && toggle.axElement) {
        ...
        SBProbeSettle(kProbeSettle);
        id<SBAXNode> afterToggle = [SBAXElementNode nodeWithElement:target.axElement];
    ...
    if (open && toggle.axElement) {
        ...
        SBProbeSettle(kProbeSettle);
        open = [self driverViewOf:[SBAXElementNode nodeWithElement:target.axElement]][@"listFound"] != nil
             && [[self driverViewOf:[SBAXElementNode nodeWithElement:target.axElement]][@"listFound"] boolValue];
    }
    report[@"leftOpen"] = @(open);
    report[@"shownAfter"] = [SBComboBoxDriver shownTextsForComboBox:[SBAXElementNode nodeWithElement:target.axElement] typed:nil] ?: @[];
    SBLog(@"probe: combobox %@ leftOpen=%d", SBLogLabel(targetLabel), open);
    return report;
}
