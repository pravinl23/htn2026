...
NSString *const SBWalkFocusElsewhere = @"elsewhere";

NSString *const SBGhostActionFill = @"fill";
NSString *const SBGhostActionSelect = @"select";
NSString *const SBGhostActionCheck = @"check";
NSString *const SBGhostActionClick = @"click";
NSString *const SBGhostActionUpload = @"upload";
...
+ (instancetype)ghostWithDictionary:(NSDictionary<NSString *, id> *)dictionary {
    ...
    if (![@[ SBGhostActionFill, SBGhostActionSelect, SBGhostActionCheck, SBGhostActionClick, SBGhostActionUpload ] containsObject:action]) return nil;
    ...
    ghost.locked = ([locked isKindOfClass:[NSNumber class]] && [locked boolValue]) || [action isEqualToString:SBGhostActionClick];
    ...
    ghost.lazy = [action isEqualToString:SBGhostActionSelect] && [lazy isKindOfClass:[NSNumber class]] && [lazy boolValue];
...
+ (NSArray<SBGhost *> *)ghostsWithDictionaries:(NSArray *)dictionaries {
    ...
    for (id entry in dictionaries) {
        SBGhost *ghost = [self ghostWithDictionary:entry];
...
- (NSInteger)keystrokes {
    if ([self.action isEqualToString:SBGhostActionFill]) return (NSInteger)self.value.length;
    return 1;
}

- (id)copyWithZone:(NSZone *)zone {
    SBGhost *copy = [[SBGhost allocWithZone:zone] init];
...
- (NSString *)description {
    ...
    return [NSString stringWithFormat:@"<SBGhost %@ %@%@%@%@%@>", self.action, self.signature, self.locked ? @" locked" : @"",
...
SBKeyDecision SBDecideTab(SBWalkSnapshot snapshot, SBKeyModifiers modifiers, BOOL isRepeat, SBHoldState *hold) {
    SBHoldState scratch = { NO, NO };
    if (!hold) hold = &scratch;
    // Disabled, untrusted, paused app: nothing is ever consumed. Shift+Tab and modified Tab are always native.
    if (!snapshot.active || modifiers != SBKeyModifierNone) {
        hold->walking = hold->halted = NO;
        return SBKeyDecisionPass;
    }
    // A press during a write is Shabang's only while focus is still in the walk: a fresh one is queued (and owns the
    // hold that follows), a repeat is dropped. Anywhere else (another app, another field) Tab stays native.
    if (snapshot.busy) {
        if (isRepeat) return hold->walking ? SBKeyDecisionSwallow : SBKeyDecisionPass;
        ...
        hold->walking = YES;
        hold->halted = NO;
        return SBKeyDecisionQueue;
    }
    if (!isRepeat) hold->walking = hold->halted = NO;
    else if (!hold->walking) return SBKeyDecisionPass;   // a hold that started as native Tab stays native
    ...
    // convenient the good case looks. Tab stays what it is: the accept key where Shabang is doing what Tab
    // already does, walking a form and filling it. Everywhere else the Shabang key is the accept key, and it
    // has no conflict surface at all (SBEventTap: a lone modifier tap, never a chord).
    if (!isRepeat && !snapshot.focusInWalk) return SBKeyDecisionPass;
    hold->walking = YES;
    if (isRepeat && hold->halted) return SBKeyDecisionSwallow;
    if (snapshot.currentLocked) {
        hold->halted = YES;   // Tab never activates a lock; the rest of this hold is swallowed
        return SBKeyDecisionPark;
    }
    return SBKeyDecisionAccept;
}

SBKeyDecision SBDecideEscape(SBWalkSnapshot snapshot, SBKeyModifiers modifiers, BOOL isRepeat, BOOL *owned) {
    BOOL scratch = NO;
    if (!owned) owned = &scratch;
    if (isRepeat) return *owned ? SBKeyDecisionSwallow : SBKeyDecisionPass;
    *owned = NO;
    if (!snapshot.active || modifiers != SBKeyModifierNone || snapshot.busy) return SBKeyDecisionPass;
    if (!snapshot.hasCurrent || !snapshot.currentVisible || !snapshot.focusInWalk) return SBKeyDecisionPass;
    *owned = YES;
    return SBKeyDecisionDismiss;
}

#pragma mark - SBWalkState

@implementation SBWalkState {
    NSMutableArray<SBGhost *> *_ghosts;
...
- (NSArray<SBGhost *> *)ghosts { return [_ghosts copy]; }
...
- (BOOL)hasUnlocked {
    for (SBGhost *ghost in _ghosts) if (!ghost.locked) return YES;
    return NO;
}
...
- (SBGhost *)ghostWithSignature:(NSString *)signature {
...
- (void)trackLock {
    SBGhost *lock = nil;
    for (SBGhost *ghost in _ghosts) if (ghost.locked) { lock = ghost; break; }
...
- (void)rescanWithGhosts:(NSArray<SBGhost *> *)ghosts {
    ...
    NSMutableArray<SBGhost *> *unlocked = [NSMutableArray array];
    ...
    NSInteger chosen = -1;
    NSString *focus = [_focusSignature isEqualToString:SBWalkFocusElsewhere] ? nil : _focusSignature;
...
- (void)accept:(NSString *)signature {
    SBGhost *ghost = [self ghostWithSignature:signature];
...
- (void)typedOver:(NSString *)signature {
    if (signature.length == 0 || [signature isEqualToString:SBWalkFocusElsewhere]) return;
...
- (void)fail:(NSString *)signature reason:(NSString *)reason {
    _error = [NSString stringWithFormat:@"%@ could not fill this field (%@)", SBProductName, reason.length ? reason : @"failed"];
    [self dismiss:signature];
}

- (void)focusMoved:(NSString *)signature {
    _focusSignature = [signature copy];
    if (!signature || [signature isEqualToString:SBWalkFocusElsewhere]) return;
    NSInteger index = [self indexOfSignature:signature];
...
- (BOOL)skipPendingCurrent {
    ...
    for (NSInteger step = 1; step <= count; step++) {
        ...
        SBGhost *ghost = _ghosts[(NSUInteger)index];
...
- (BOOL)focus:(NSString *)focus isInWalkOf:(SBGhost *)current {
    if (!focus) return YES;   // the window itself
    if ([focus isEqualToString:SBWalkFocusElsewhere]) return NO;
...
- (SBWalkSnapshot)snapshotForFocus:(NSString *)focus active:(BOOL)active currentVisible:(BOOL)currentVisible busy:(BOOL)busy {
    SBGhost *current = self.current;
    SBWalkSnapshot snapshot = { 0 };
    ...
    snapshot.focusOnField = focus != nil && ![focus isEqualToString:SBWalkFocusElsewhere];
    snapshot.busy = busy;
    return snapshot;
}
...
- (BOOL)shouldConsumeTabWithModifiers:(SBKeyModifiers)modifiers isRepeat:(BOOL)isRepeat focusSignature:(NSString *)focusSignature
                       currentVisible:(BOOL)currentVisible hold:(SBHoldState *)hold {
    SBWalkSnapshot snapshot = [self snapshotForFocus:focusSignature active:YES currentVisible:currentVisible busy:NO];
    return SBKeyDecisionConsumes(SBDecideTab(snapshot, modifiers, isRepeat, hold));
}

- (BOOL)shouldConsumeEscapeWithModifiers:(SBKeyModifiers)modifiers focusSignature:(NSString *)focusSignature currentVisible:(BOOL)currentVisible {
    SBWalkSnapshot snapshot = [self snapshotForFocus:focusSignature active:YES currentVisible:currentVisible busy:NO];
    return SBKeyDecisionConsumes(SBDecideEscape(snapshot, modifiers, NO, NULL));
}
