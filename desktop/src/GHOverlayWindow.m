...
@interface GHOverlayPanel : NSPanel
@end

@implementation GHOverlayPanel
...
static NSPanel *GHMakeOverlayPanel(CGRect frame, CALayer *root, BOOL excludedFromCapture) {
    NSPanel *panel = [[GHOverlayPanel alloc] initWithContentRect:frame
    ...
    panel.hidesOnDeactivate = NO;  // NSPanel hides with its app by default; Ghost is never the active app
    panel.canHide = NO;
    panel.movable = NO;
...
@interface GHOverlaySurface : NSObject
@property (nonatomic, strong, nullable) NSPanel *panel;
@property (nonatomic, strong) CALayer *root;
@property (nonatomic, strong) NSMutableDictionary<NSString *, GHOverlayItemLayer *> *layers;
@property (nonatomic, copy) NSArray<GHDrawItem *> *items;
...
@implementation GHOverlaySurface
...
- (void)applyItems:(NSArray<GHDrawItem *> *)items fresh:(BOOL)fresh force:(BOOL)force reduceMotion:(BOOL)reduceMotion {
    GHOverlayDiff *diff = [GHOverlayDiff diffFromItems:self.items toItems:items];
    ...
    for (GHDrawItem *item in diff.added) {
        GHOverlayItemLayer *layer = [GHOverlayItemLayer layerForItem:item];
        self.layers[item.key] = layer;
        [self.root addSublayer:layer];
        [layer applyItem:item glide:NO reduceMotion:reduceMotion];
        if (!reduceMotion && !fresh) GHFadeIn(layer, 0.14);
        self.targets[item.key] = item.targetSignature;
    }
    NSArray<GHDrawItem *> *repaint = force ? [diff.changed arrayByAddingObjectsFromArray:diff.unchanged] : diff.changed;
    for (GHDrawItem *item in repaint) {
    ...
    self.items = items;
    self.root.hidden = NO;
    if (fresh && !reduceMotion && items.count > 0) GHFadeIn(self.root, 0.12);
}
...
@implementation GHOverlayWindow {
    NSMutableArray<GHOverlaySurface *> *_surfaces;
    GHOverlayModel *_lastModel;
    BOOL _fixedLayout;
    BOOL _hidden;
    BOOL _dead;
}
...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        _hidden = YES;
        _layout = [GHScreenLayout currentLayout];
...
- (void)invalidate {
    ...
    [NSNotificationCenter.defaultCenter removeObserver:self];
    [NSWorkspace.sharedWorkspace.notificationCenter removeObserver:self];
    for (GHOverlaySurface *surface in _surfaces) {
        [surface clear];
        [surface.panel close];
    }
...
- (void)rebuildSurfaces {
    for (GHOverlaySurface *surface in _surfaces) [surface.panel close];
    [_surfaces removeAllObjects];
    for (NSUInteger i = 0; i < self.layout.count; i++) {
        CGRect frame = [self.layout frameAtIndex:i];
        GHOverlaySurface *surface = [[GHOverlaySurface alloc] initWithSize:frame.size];
        if (!_fixedLayout) surface.panel = GHMakeOverlayPanel(frame, surface.root, self.excludedFromCapture);
...
- (void)renderInput:(GHOverlayInput *)input {
    [self render:[GHOverlayModel modelWithInput:input layout:self.layout]];
}

- (void)applyModel:(GHOverlayModel *)model force:(BOOL)force {
...
- (void)hideImmediately {
    ...
    for (GHOverlaySurface *surface in _surfaces) [surface.panel orderOut:nil];
}
...
- (void)screensChanged:(NSNotification *)note {
    ...
    GHScreenLayout *now = [GHScreenLayout currentLayout];
