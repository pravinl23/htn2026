// SBSafetyChecking: the two shared safety rules (shared/src/sensitive.ts, shared/src/locks.ts) as seen
// from native code. SBCore conforms by forwarding to GhostCore.isSensitive / GhostCore.isLockedAction;
// tests use a fake. Capture ORs these answers with its own native patterns, so a core that failed to
// load can only make Shabang stricter, never looser.
#import <Foundation/Foundation.h>

NS_ASSUME_NONNULL_BEGIN

@protocol SBSafetyChecking <NSObject>
