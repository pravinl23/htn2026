// SBHarness: the agent-drivable test and debug harness (docs/desktop-realworld.md section 2).
//
//   --trust                       { "trusted": bool }
...
//   --next                        what Shabang would PROPOSE here (docs/anywhere.md): page kind, ranked roles, the top
...
//   --probe-combobox "How did"    open ONE combo box, write down what the tree then shows, close it again (SBProbe)
...
//   - The only key this file can post is Tab (SBAutotabKeyPosting has no other method). Never Return, Enter or Space.
...
// --autotab travel to that agent as a JSON file (SBHarnessChannel) and the agent writes --out. Without a running
...
extern NSString *const SBHarnessModeTrust;
extern NSString *const SBHarnessModeDump;
extern NSString *const SBHarnessModeDumpTree;
extern NSString *const SBHarnessModeNext;
extern NSString *const SBHarnessModeAutotab;
/// Take the ghost that is on screen right now, exactly as the Shabang key does. The ONE harness mode that
/// actuates: it presses a real control in a real app. Read-only everywhere else.
extern NSString *const SBHarnessModeAccept;
extern NSString *const SBHarnessModeProbeComboBox;

extern const NSInteger SBHarnessMaxAutotabCount;        // 200
extern const NSInteger SBHarnessDefaultIntervalMs;      // 450
extern const NSInteger SBHarnessDefaultDepth;           // 60
extern const NSUInteger SBHarnessMaxTreeNodes;          // 8000
extern const NSUInteger SBHarnessMaxTextLength;         // 120: titles, descriptions and static text are cut here
extern const NSTimeInterval SBHarnessRequestMaxAge;     // 120 s: an older request file is dropped, never run

#pragma mark - request and response

/// One harness invocation. Built from argv in the launched process; travels as JSON to a running agent.
@interface SBHarnessRequest : NSObject
...
BOOL SBHarnessLabelsMeetExpectation(NSArray<NSString *> *_Nullable labels, NSString *_Nullable expectation);

/// { "error": "not trusted", "trusted": false }
NSDictionary<NSString *, id> *SBHarnessNotTrustedResponse(void);
NSDictionary<NSString *, id> *SBHarnessErrorResponse(NSString *code, NSString *_Nullable detail);
...
/// nobody else can read it. Refuses a path that fails SBHarnessProblemWithOutPath. With a nil path the JSON goes
/// to stdout.
BOOL SBHarnessWriteResponse(NSDictionary<NSString *, id> *response, NSString *_Nullable outPath);
...
NSString *_Nullable SBHarnessProblemWithOutPath(NSString *_Nullable path);
...
BOOL SBHarnessRemoveOldAnswer(NSString *_Nullable path);
...
@interface SBHarnessChannel : NSObject
...
extern NSString *const SBHarnessRequestNotification;

/// The agent's side: watches the channel, runs one request at a time, writes each answer to the request's --out.
@interface SBHarnessServer : NSObject
- (instancetype)init NS_UNAVAILABLE;
- (instancetype)initWithChannel:(SBHarnessChannel *)channel controller:(SBController *_Nullable (^)(void))controller NS_DESIGNATED_INITIALIZER;
- (void)start;
- (void)stop;
/// Tests: how a request is performed (default: +[SBHarness performRequest:controller:completion:]).
@property (nonatomic, copy) void (^perform)(SBHarnessRequest *request, void (^completion)(NSDictionary<NSString *, id> *response));
...
@interface SBHarnessTree : NSObject
...
/// inside them is input. Other AXStaticText is page text and is kept (cut to SBHarnessMaxTextLength) as `text`.
...
+ (NSDictionary<NSString *, id> *)treeFromNode:(id<SBAXNode>)root
                                      maxDepth:(NSUInteger)maxDepth
                                      maxNodes:(NSUInteger)maxNodes
                                       actions:(nullable NSArray<NSString *> *_Nullable (^)(id<SBAXNode> node))actions
...
/// What the loop reads. SBController is one; tests pass a fake.
@protocol SBAutotabSubject <NSObject>
...
@protocol SBAutotabKeyPosting <NSObject>
/// One unmodified Tab press (down + up). NO when it could not be posted.
- (BOOL)postTab;
@end

/// Live poster: a plain HID-state event source, no SBSyntheticEventUserData tag, no modifier flags, so the press
/// goes through Shabang's own event tap exactly like the user's. Key code 48 is hard-wired.
@interface SBHarnessTabPoster : NSObject <SBAutotabKeyPosting>
...
@interface SBAutotabRunner : NSObject
- (instancetype)init NS_UNAVAILABLE;
- (instancetype)initWithSubject:(id<SBAutotabSubject>)subject poster:(id<SBAutotabKeyPosting>)poster NS_DESIGNATED_INITIALIZER;
...
/// presses in a row that Shabang did not consume -> "stalled"; over budget -> "timeout"; then `precondition`. A press
...
/// SBHarnessNotTrustedResponse at once.
+ (void)performRequest:(SBHarnessRequest *)request controller:(nullable SBController *)controller
