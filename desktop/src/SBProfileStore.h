// SBProfileStore: ~/Library/Application Support/Shabang/profile.json, settings.json and answers.json.
...
// keyed by the question rather than by the site. It is written only by Shabang, it never leaves the machine, and a
// missing, empty or corrupt file simply means "nothing learned yet" -- it never stops Shabang from proposing.
...
/// by other users, not even for a moment). Used for everything Shabang keeps under Application Support.
BOOL SBWritePrivateFile(NSString *path, NSData *data, NSError *_Nullable *_Nullable error);

/// Posted on the main queue after profile.json or settings.json changed on disk or through this class.
extern NSNotificationName const SBProfileStoreDidChangeNotification;
...
/// a reason code, never the path). They never leave the machine: not in /v1/predict/form, not in /v1/shabang-text.
extern NSString *const SBProfileResumePathKey;        // "resumePath"
extern NSString *const SBProfileCoverLetterPathKey;   // "coverLetterPath"
/// The usable absolute path for a file fact, or nil with *problem set to a short code.
NSString *_Nullable SBUsableProfileFilePath(NSString *_Nullable raw, NSString *_Nullable *_Nullable problem);

@interface SBProfileStore : NSObject
...
- (instancetype)initWithDirectory:(NSString *)directory core:(nullable SBCore *)core NS_DESIGNATED_INITIALIZER;
- (instancetype)initWithCore:(nullable SBCore *)core;
...
/// The same as a JSON string, ready for SBCore; "" when there is nothing learned.
...
/// YES for the built-in list, the user's list, and Shabang itself. nil counts as paused (unknown app).
