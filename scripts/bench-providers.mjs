...
if (!process.env.GHOST_BENCH_CHILD) {
  ...
  const child = spawnSync(process.execPath, ["--import", "tsx", SELF, ...process.argv.slice(2)], {
    ...
    env: { ...process.env, GHOST_BENCH_CHILD: "1", GHOST_WARMUP: "0", GHOST_PROVIDER: "", GHOST_DECISION_PROVIDER: "", GHOST_TEXT_PROVIDER: "" },
  });
...
function configFor(decision, text, extra = {}) {
  return loadConfig({ ...process.env, GHOST_PROVIDER: undefined, GHOST_DECISION_PROVIDER: decision, GHOST_TEXT_PROVIDER: text, GHOST_WARMUP: "0", ...extra });
}
...
function markdown(results, meta) {
  const lines = [
    ...
    "",
    `Task: ${FORM.describe}, ${N} decisions per provider through the server's provider factories${DRAFTS ? `, and ${DRAFTS} streamed ghost-text drafts (at most 360 characters) per text provider through \`POST /v1/ghost-text\`` : ""}. Decision latency is wall clock around \`provider.decide()\`, so for Baseten it is the time until the K-th valid sample of the hedged vote, inside the production deadline of 2.3 s. Percentiles are nearest-rank, so with N=${N} the p95 is the slowest successful call.`,
    "",
