...
No Jev credential is present in the current checkout or process environment. With `TYPESAFE_API_KEY` or `AI_GATEWAY_API_KEY`, workflow prediction uses the existing provider; otherwise the explicit demo uses a deterministic local selector. Production no-key requests are inert and return only `no_action`/local actions.
