...
## 2. Next-action prediction (Stage 5: implemented)

- After each user action settles (300 ms debounce) and no form ghosts are pending, the content script sends up to 60 visible controls in or near the viewport plus the last 20 actions to `POST /v1/predict/next` (see docs/server-api.md). Controls include native fields/buttons/links and app-style ARIA tabs, menus, options, tree items, switches, summaries and custom click targets. Before the cap, generic semantic and structural salience puts task controls (editable search, primary/form and commerce actions, media controls, main/dialog content and heading-named result links) ahead of passive site chrome, carousel, pagination and filter controls. No hostname or site selector participates.
- **Episodic memory**: background stores `(stateSummary, action)` pairs where `stateSummary = pathPattern + the last 3 event shape keys` (see 3.1). An exact state wins at confidence 0.75 (once) / 0.9 (twice or more). For a state never seen before, Ghost falls back to the newest compatible action demonstrated on the same origin at 0.65 / 0.8; another website's history can never enter that fallback. Repeated result/feed groups carry a value-free structural group, so memory can generalize from an old item to the current first-ranked item even when item labels changed. The model also receives up to 5 exact/recent same-origin memories.
- **Dynamic sites**: candidate capture is retried as a page hydrates or replaces controls. DOM mutations use a 300 ms quiet period with a 1.2 s ceiling, so stable pages ask once and continuously mutating SPAs cannot postpone prediction forever.
- Learned behavior wins. In an unseen state, Ghost still shows a deliberately low-confidence semantic best guess; recent transitions advance from search to results and from media playback to viewing controls instead of repeating the previous action. If the user does something else, that actual action—not the rejected suggestion—is recorded for the next visit. Tab clicks the chosen target unless locked. Locked targets (including place-order/submit actions) can be suggested but only get the lock badge and focus. With zero safe candidates Ghost leaves Tab native.
- Cross-page values: when a field is focused/empty and its label semantically matches a recent page fact or a recently picked value (e.g. a calendar slot picked on `/calendar`, then a reply box on `/mail`), propose a fill ghost. For the mail/calendar demo the reply draft comes from `/v1/ghost-text` with `pageContext.description` containing the email text and `facts.pickedSlot`.
