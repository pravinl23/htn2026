...
- Learned behavior wins. In an unseen state, Ghost still shows a deliberately low-confidence best guess; if the user does something else, that actual action—not the rejected suggestion—is recorded for the next visit. Tab clicks the chosen target unless locked. Locked targets only get the lock badge and focus. With zero safe candidates Ghost leaves Tab native.
