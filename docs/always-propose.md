...
**If Shabang can see anything actionable, it proposes one. It never goes quiet because it is unsure.**
...
| Below the user's threshold (anything else, including a pure prior on a page Shabang has never seen) | `long-shot` | Dimmer ghost, "guess" chip, and the reason in the HUD. Still one key to accept, still one keystroke to ignore. |
...
Proposing is not doing. Shabang may propose anything, including an irreversible action, but it still never *performs* one without an explicit human press: Submit, Send, Pay, Delete and friends keep their lock, and holding the accept key stops at every lock and every guess. A wrong guess is one keystroke to fix; an unasked-for purchase is not.
...
2. On a page Shabang has never seen, page-kind priors alone are enough to propose (`docs/anywhere.md`).
