...
5. Work the explicit **Top priority** in PLAN.md first, then remaining unchecked items in dependency order. The current top priority is the extension invoice-loop vertical slice; do not detour into another demo surface or live executor before it works. For each item: implement it, add or extend tests, run the full test suite, fix failures, check the box in PLAN.md, commit with a clear message, run `git pull --rebase origin main`, and push to main. Push after every completed item so work lands in small chunks.
